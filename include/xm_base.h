...
#define	SDS_POWERACC			3		// 关机,等待ACC上电重启, 用于停车监控

// 电池状态事件
#define	DEVCAP_VOLTAGE_WORST		8
...
#define	DEVCAP_SDCARDSTATE				0x07	
			// SD卡状态检测 (卡状态不等同于卡事件, 卡事件比卡状态多)
			// 1	SD卡未插入
			// 2	SD卡插入，且读写允许
			// 3	SD卡插入，无法识别的文件系统, 文件系统异常等
			// 4	SD卡插入，数据卡不能识别，需更换
	#define	DEVCAP_SDCARDSTATE_UNPLUG						1	// 数据卡拔出
	#define	DEVCAP_SDCARDSTATE_INSERT						2	// 数据卡读写模式(正常模式)
	#define	DEVCAP_SDCARDSTATE_FS_ERROR						3	// 数据卡已识别(读写模式)，卡文件系统已安装
																// 无法在数据卡上创建保存视频记录的目录， 
																// 存在与目录同名的文件时，将其重命名(自动修复)
																// 无法自动修复时需要提示"格式化或者换卡"
															
	#define	DEVCAP_SDCARDSTATE_INVALID						4	// 数据卡不能识别，需更换(FS_MOUNT失败)
...
#define	DEVCAP_GPSBD					0x11
	...
	#define	DEVCAP_GPSBD_CONNECT_LOCATE_OK			1	
...
#define	DEVCAP_OSD							0x14	

#define	DEVCAP_BACKLIGHT_LEVEL			0x25	// 背光级别设置
	#define	DEVCAP_BACKLIGHT_LEVEL_0		0		// 背光0%
	#define	DEVCAP_BACKLIGHT_LEVEL_10		10		// 背光10%
	#define	DEVCAP_BACKLIGHT_LEVEL_50		50		// 背光50%
	#define	DEVCAP_BACKLIGHT_LEVEL_100		100	// 背光100%亮度, 占空比为100%		
