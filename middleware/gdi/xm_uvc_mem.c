...
#define XM_UVC_MEM_SIZE		0x300000		// 3M字节uvc 动态内存空间
...
__no_init static CHAR		_heap_mem[XM_UVC_MEM_SIZE];
...
void usb_mem_init (void)
{
	...
	addr = _heap_mem;
	p = addr + PTRSIZE;
	np = p + XM_UVC_MEM_SIZE - 2 * PTRSIZE;
	...
	_heap_empty		= p;
	_heap_bottom	= p;
	_heap_top		= addr + XM_UVC_MEM_SIZE;
}

void usb_mem_exit (void)
{

}

// 保存uvcmem系统的运行环境
void uvc_mem_save_environment (unsigned char *buff, unsigned int size)
{
...
// 恢复uvcmem系统的运行环境
void uvc_mem_restore_environment (unsigned char *buff, unsigned int size)
{
...
void *	XM_UVC_malloc	(int size)
{
	...
	XM_printf ("### XM_UVC_malloc failed, size=%d, len=%d\n", size, len);
...
void *	XM_UVC_calloc	(int size)
{
	void *mem = XM_UVC_malloc (size);
