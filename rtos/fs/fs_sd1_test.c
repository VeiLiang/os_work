...
int _fs_sd1_function_test (unsigned int sd_dev_index)
{
	FS_FILE *fp;
	FS_DISK_INFO Info;
	int ret = 0;
	...
	while(1){
		...
		sprintf (dev_name, "mmc1:%d:", sd_dev_index);
...
int fs_sd1_function_test (unsigned int sd_dev_index)
{
	char dev_name[32];
		if(sd_dev_index >= SDMMC_DEV_COUNT)
		{
	...
	do
	{
		...
		_fs_sd1_function_test (sd_dev_index);
...
int fs_sd1_sector_read_test (unsigned int sd_dev_index)
{
	char dev_name[32];
		if(sd_dev_index >= SDMMC_DEV_COUNT)
		{
...
#define SECTOR_COUNT	32
		...
		{
			char file_name[32];
			sprintf (file_name, "%s\\SEC%03d.BIN", dev_name, j);
			printf ("write file (%s)\n", file_name);
