...
static struct input_dev	g_goodix_dev;
...
static DECLARE_WORK(goodix_init_work, goodix_init_events,NULL);
static DECLARE_WORK(goodix_resume_work, goodix_resume_events,NULL);
...
#define CTP_IRQ_NUMBER         0// (config_info.int_number)
#define CTP_RST_NUMBER         1// (config_info.wakeup_gpio.gpio)
...
void goodix_ts_reset(void);
void goodix_ts_gpioinit(void);
static int goodix_request_irq(struct goodix_ts_data *ts);
static void goodix_request_io_port(struct goodix_ts_data *ts);
...
//static const unsigned short normal_i2c[2] = {0x14,I2C_CLIENT_END};   jordan.chen 20141014
...
int ctp_wakeup(int status,int ms)
{
	dprintk(DEBUG_INIT,"***CTP*** %s:status:%d,ms = %d\n",__func__,status,ms);
       GPIO_9_8_DATADIR |= (1 << 5);     // reset pin input
	if (status == 0) 
	{
...
void gtp_set_int_value(int status)
{
	...
	#else
	GPIO_9_8_DATADIR |= (1<<TS_IRQ_NUM);  //int pin output
	if(status == 1)
	{
	        GPIO_9_8_DATAOUT |= (1 << TS_IRQ_NUM);
	}
	else
	{
	        GPIO_9_8_DATAOUT&=~ (1 << TS_IRQ_NUM);
...
{       
     ...
     //   gtp_set_int_value(1);    jordan.chen 20171014
     //   msleep(2);
...
s32 gtp_i2c_read(struct i2c_client *client, u8 *buf, s32 len)
{
	...
	if((retries >= 5))
	{
        ...
        s32 ret=-1;
        //GTP_INFO("I2C Read: devAddr=%x  buf[0]=%x buf[1]=%x  startAddr=%x  len=%x\n", GOODIX_I2C_DEV_ADDR,  buf[0],buf[1],(buf[1]+(buf[0]*256)), len);
	 ret = i2c_read(GOODIX_I2C_DEV_ADDR, (buf[1]+(buf[0]*256)), 2, &buf[2],  len - GTP_ADDR_LENGTH);
...
s32 gtp_i2c_write(struct i2c_client *client,u8 *buf,s32 len)
{
	...
	ret = i2c_write(GOODIX_I2C_DEV_ADDR, (buf[1]+(buf[0]*256)), 2, &buf[2], len - GTP_ADDR_LENGTH);
	if(ret == 0)
	{
              return 1;
	}
...
s32 gtp_send_cfg(struct i2c_client *client)
{
	...
	s32 retry = 0;
	struct goodix_ts_data *ts = ts_init;//i2c_get_clientdata(client);
...
void gtp_irq_disable(struct goodix_ts_data *ts)
{
        ...
        if (!ts->irq_is_disable)
	{
			   ...
			   #else
			    GPIO_9_8_INTEN =0; //must be 0 to disable the interrupt, or it will continously produce interrupt 
                         disable_irq(TS_IRQ);
			   #endif
        }
...
#if GTP_ICS_SLOT_REPORT
           /*
	    input_report_abs(ts->input_dev, ABS_MT_POSITION_X, y*1024/600);
	    input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, (1024-x)*600/1024);
...
#if GTP_ICS_SLOT_REPORT
           /*
	    input_report_abs(ts->input_dev, ABS_MT_POSITION_X, (600-y)*1024/600);
	    input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, x*600/1024);
	    input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR, w);
	    input_report_abs(ts->input_dev, ABS_MT_WIDTH_MAJOR, w);
	    input_report_abs(ts->input_dev, ABS_MT_TRACKING_ID, id);
	    input_mt_sync(ts->input_dev);
...
static void goodix_ts_work_func(struct work_struct *work)
{
	...
	if (ts->enter_update)
	{
	     //  GTP_DEBUG("step:00000000000000000\n");
		return;
	}
       //hw interrupt setting
       GPIO_9_8_INTEN |= 0x1 << TS_IRQ_NUM; 
       enable_irq(ts->irq);
	 //GTP_DEBUG("step:1111111111111111\n");	
...
#endif
        //GTP_DEBUG("step:222222222222222222222222\n");	
	...
	if (ret < 0)
	{
       ...
       //GTP_DEBUG("step:3333333333333333333333\n");	
...
#endif
       //GTP_DEBUG("step:55555555555555\n");	
	if((finger & 0x80) == 0)
	{
	       GTP_DEBUG("exit_work_func\n");	
		goto exit_work_func;
	}
       //GTP_DEBUG("step:666666666666666\n");	
	...
	if (touch_num > 1)
	{
       ...
       //GTP_DEBUG("step:777777777777777\n");	
#if GTP_HAVE_TOUCH_KEY
...
#else
        GTP_DEBUG("touch_num = %d, pre_touch = 0x%x\n",touch_num, pre_touch);
        if (touch_num ) 
	 {
	...
	input_sync(ts->input_dev);
       //GTP_DEBUG("step:8888888888888888\n");	
...
static irqreturn_t goodix_ts_irq_handler(int irq, void *dev_id)
{
	...
	//GTP_DEBUG("==========------TS Interrupt-----============\n");

#if 1
	//disable irq first 	
	GPIO_9_8_INTEN =0; //must be 0 to disable the interrupt, or it will continously produce interrupt 
	irq_status = GPIO_9_8_INT_STAT & (0x1 << TS_IRQ_NUM);
	GPIO_9_8_INT_CLR = irq_status; 
	disable_irq(TS_IRQ);
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
	...
	//GTP_GPIO_OUTPUT(GTP_INT_PORT, (client->addr)==0x14);   jordan.chen 20171014
	GTP_GPIO_OUTPUT(GTP_INT_PORT, 0);
...
static s32 gtp_esd_recovery(struct i2c_client *client)
{
	...
	ts =ts_init;// i2c_get_clientdata(client);
...
static void goodix_init_events (struct work_struct *work)
{
	...
	ret= goodix_request_input_dev(ts_init);
	if (ret < 0)
	{
	        GTP_ERROR("request input dev failed\n");
	        return ret;        
	}
	...
	goodix_request_io_port(ts_init);
	ret=goodix_request_irq(ts_init);
	if (ret != 0)
	{
		GTP_ERROR("request irq error, use timer\n");
		ts_init->use_irq = 0;
	}
...
static void goodix_resume_events (struct work_struct *work)
{
	...
	{
		//hrtimer_start(&ts_init->timer, ktime_set(1, 0), HRTIMER_MODE_REL); jordan.chen 20171014
		;
	}
...
None.
 *******************************************************/
static int goodix_ts_suspend(struct i2c_client *client)//, pm_message_t mesg    Jordan.chen 20171014
{
	struct goodix_ts_data *ts;
	s8 ret = -1;
	ts = ts_init;//i2c_get_clientdata(client);
	...
	if (true == ts->is_suspended) {
		...
		{
			//hrtimer_cancel(&ts->timer);  jordan.chen 20171014
		}
...
static void gtp_esd_check_func(struct work_struct *work)
{
	...
	ts =ts_init;// i2c_get_clientdata(i2c_connect_client);
...
static int goodix_request_input_dev(struct goodix_ts_data *ts)
{
    ...
    ts->input_dev = &g_goodix_dev;
    input_dev = ts->input_dev;
    init_input_dev(ts->input_dev);  
...
static int goodix_request_irq(struct goodix_ts_data *ts)
{
    ...
    ts->irq = TS_IRQ;//IRQ_SIRQ0;

    err = request_irq(ts->irq, goodix_ts_irq_handler, SA_SHIRQ, "gt9xxf_ts", ts); //IRQ_TYPE_EDGE_FALLING
	
    if (err < 0) 
    {
		ts->use_irq = 0;
		GTP_ERROR("goodix_request_irq: request irq failed\n");
		return err;
    } 
    else
    {
...
void goodix_ts_reset(void)
{
       //Sel I2C addr 0XBA 0XBB
	GPIO_9_8_DATADIR |= (1 << 5);     // reset pin output
	GPIO_9_8_DATADIR |= (1<<TS_IRQ_NUM);  //int pin output
...
void goodix_ts_gpioinit(void)
{
	 ...
	 */
	 msleep(100);
	GPIO_9_8_DATADIR &= (~(1<<TS_IRQ_NUM));
	//gpio interrupt setting
	GPIO_9_8_INTEN |= 0x1 << TS_IRQ_NUM;
	GPIO_9_8_INT_TYPE |= (GPIO_INT_EDGE << TS_IRQ_NUM) ;
	GPIO_9_8_INT_POL |= 0 << TS_IRQ_NUM;
	GPIO_9_8_INTMASK &= (~(0x1 <<TS_IRQ_NUM));
}
/*******************************************************    
Function:
...
static int goodix_ts_init(void)
{
	...
	memset(ts, 0, sizeof(*ts));
	//INIT_WORK(&ts->work, goodix_ts_work_func,&ts->work);
	...
	GTP_DEBUG("goodix_wq   end!!!!!!!!!!");

	goodix_init_events(&ts->work);
	...
	GTP_DEBUG("goodix_resume_wq   end!!!!!!!!!!");
