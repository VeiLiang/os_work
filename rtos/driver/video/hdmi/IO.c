...
extern int i2c_reg8_write (unsigned int slvaddr, unsigned int ucDataOffset, unsigned char * data, unsigned char size);
extern int i2c_reg8_read (unsigned int slvaddr, unsigned int ucDataOffset, unsigned char *data,unsigned char size);


BOOL i2c_write_byte( BYTE address,BYTE offset,BYTE byteno,BYTE *p_data,BYTE device )
{
	if(i2c_reg8_write (address, offset, p_data, byteno) < 0)
		return 0;
	return 1;
}
...
{
 	if(i2c_reg8_read (address, offset, p_data, byteno) < 0)
		return 0;
    return 1;
}
