...
{
    if(AP_GetColor_Saturation() < 41)
        return AP_GetColor_Saturation() *3;
    else
        return 41*3 + (AP_GetColor_Saturation() -41)*2;
}
...
void N3_SetCH4_VD2(void)
{
	...
	Write_I2C( DEV_ADDR, 0xFF, NULL, BANK1,1);
	// ch3 2 vd2
	Write_I2C( DEV_ADDR, 0xC2, NULL, 0x33,1);
	I2CWriteByte(DEV_ADDR, 0xFF, 0x06);
			// HV ch3 to vd2
...
VOID N3_SetOutputChannel_Param(VOID)
{
	N3_SetCH1_VD1();
	N3_SetCH2_VD2();
	I2CWriteByte(DEV_ADDR, 0xFF, 0x01);
...
VOID N3_SetOutPutChannel_Data(VOID)
{
	//通道0
	N3_SetCH1_VD1();
	I2CWriteByte(DEV_ADDR, 0xFF, 0x01);
...
void N3_SetOutputChannel(  n123_init_param *pInitParam, int ch )
{
	int tmp;
...
int N3_Init( n123_init_param *pInitParam )
{
	...
	else if(pInitParam->fmt_res == VDEC_N123_FMT_1080P)
	{
		...
		int i;
		printf("11111111111111111111111111111111111 VDEC_N123_FMT_1080P \n");
		N3_Init_ch(3,_0_30P_1_25P,0);	//CH1/2/3/4 1080P
		N3_Init_ch(2,_0_30P_1_25P,1); //CH2 720P
		#if 0
		for(i=0;i<4;i++)
		{
			if(N3_Mode==0x00)
...
{
	return 720;//Sensor_Output_Height;
}

u32_t NVP6214_get_video_width (void)
{
	return 1280;//Sensor_Output_Width;
}
...
static unsigned int  Sensor_Output_Width_0=1280;
static unsigned int  Sensor_Output_Height_0=720;
static unsigned char Sensor_Output_Format_0=AHD_25FPS;
static unsigned char Sensor_Pre_Output_Format_1=NO_DETECT;
static unsigned int  Sensor_Output_Width_1=1280;
static unsigned int  Sensor_Output_Height_1=720;
static unsigned char Sensor_Output_Format_1=AHD_25FPS;
...
void N3_VideoFormat_Detection(  n123_init_param *pInitParam, int ch )
{
    ...
    printf("11111111111 video_format: %x video_format_Pre: %d\n",video_format,video_format_Pre);
	#if 0
    ...
    if(((video_format != Sensor_Pre_Output_Format_0) || Manual_Switch)&&Close_Audio_Sound)
    {
       	Sensor_Output_Format_0 = video_format;
		Sensor_Pre_Output_Format_0 = Sensor_Output_Format_0;
		if(Sensor_Output_Format_0 == AHD_30FPS)
		{
	   		Sensor_Output_Width_0 = 1280;
	   		Sensor_Output_Height_0 = 720;
		...
		else if(Sensor_Output_Format_0 == AHD_25FPS)
		{
	   		Sensor_Output_Width_0 = 1280;
	   		Sensor_Output_Height_0 = 720;
		...
		else if(Sensor_Output_Format_0 == AHD_60FPS)
		{
	   		Sensor_Output_Width_0 = 1280;
	   		Sensor_Output_Height_0 = 720;
		...
		else if(Sensor_Output_Format_0 == AHD_50FPS)
		{
	   		Sensor_Output_Width_0 = 1280;
	   		Sensor_Output_Height_0 = 720;
		...
		else if(Sensor_Output_Format_0 == AFHD_30FPS)
		{
	   		Sensor_Output_Width_0 = 1920;
	   		Sensor_Output_Height_0 = 1080;
		...
		else if(Sensor_Output_Format_0 == AFHD_25FPS) //默认1080P 25fps
		{
	   		Sensor_Output_Width_0 = 1920;
	   		Sensor_Output_Height_0 = 1080;
			init_param.fmt_fps = VDEC_N123_FMT_25P;
        ...
        if(Sensor_Output_Format_0 == NO_DETECT) {
		...
		printf("111111 Sensor_Output_Format_0: %x Sensor_Pre_Format: %x \n",Sensor_Output_Format_0,Sensor_Pre_Format);
		if((((Sensor_Output_Format_0 != NO_DETECT)&& (Sensor_Output_Format_0 !=Sensor_Pre_Format))  || Manual_Switch)&&Close_Audio_Sound)
		{
			...
			printf("11111111111111111NO_DETECT init_param.fmt_fps: %d N3_Mode: %d\n",init_param.fmt_fps,N3_Mode);
			if(init_param.fmt_fps == VDEC_N123_FMT_25P) {
				N3_Init_ch(N3_Mode,1,0);	//CH1/2/3/4 1080P
				N3_SetOutPutChannel_Data();
			}
			else if(init_param.fmt_fps == VDEC_N123_FMT_30P) {
				N3_Init_ch(N3_Mode,0,0);	//CH1/2/3/4 1080P
			}
			
			Sensor_Pre_Format = Sensor_Output_Format_0;
			OS_Delay(10);
...
{
    OS_SignalEvent(KEY_EVENT, &TCB_NVP6124B_Task); /* 通知事件 */
}
