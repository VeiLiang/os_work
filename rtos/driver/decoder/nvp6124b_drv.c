...
#endif
static unsigned char Sensor_Output_Height = 0;
static unsigned char Sensor_Output_Width = 0;
static unsigned char Sensor_Output_Format=AHD_25FPS;

static unsigned int  Sensor_Output_0_Width=1280;
static unsigned int  Sensor_Output_0_Height=720;
static unsigned int  Sensor_Output_1_Width=1280;
static unsigned int  Sensor_Output_1_Height=720;


static unsigned char Sensor_Pre_Output_Format=NO_DETECT;
static unsigned char Sensor_Pre_Output_Format_0=NO_DETECT;
		...
		switch(fmt_res)
		{
			case VDEC_N123_FMT_SH720:
					...
					if(N3_Mode==0x01||N3_Mode==0x03||N3_Mode==0x05)
					{
						...
						I2CWriteByte(DEV_ADDR, 0x34,_0ntsc1pal?0x40:0x00);  //20170525: ntsc: 0x03
						...
						I2CWriteByte(DEV_ADDR, 0x6c,0x0F);
					...
					else if(N3_Mode==0x02)
					{
						...
						I2CWriteByte(DEV_ADDR, 0x6c,0x0F);  //0x0F: bt656 60hz; 0x17~0x1f: bt601 30hz; 0x10~0x16: bt601 60hz 
						I2CWriteByte(DEV_ADDR, 0x6D,0xFF); //change vertical shaking
						printf("N3 720h each BT601 ch%d %s setting  \n",ch,_0ntsc1pal?"PAL":"NTSC");
					}
					#endif
			...
			case VDEC_N123_FMT_1080P:
					...
					I2CWriteByte(DEV_ADDR, 0x9C,_0ntsc1pal?0x00:0x00);
					I2CWriteByte(DEV_ADDR, 0x9D,_0ntsc1pal?0xC1:0xC1); //行同步的开始
					I2CWriteByte(DEV_ADDR, 0x9E,_0ntsc1pal?0x0f:0x0f);
					I2CWriteByte(DEV_ADDR, 0x9F,_0ntsc1pal?0xC0:0xC0); //行同步的起始位置 影响输出的颜色
					//change vdelay
...
/*切换前面摄像头在CH1输入作为VD1输出数据*/
void N3_SetCH1_VD1(void)
{
     // DEV_ADDR = pInitParam->devaddr;
	Write_I2C( DEV_ADDR, 0xFF, NULL, BANK1,1);
	 Write_I2C( DEV_ADDR, 0xC0, NULL, 0x00,1);
	I2CWriteByte(DEV_ADDR, 0xFF, 0x05);
	// HV ch1 to vd1
	I2CWriteByte(DEV_ADDR, 0x3C, 0x10);

}
/*切换前面摄像头在CH2输入作为VD1输出数据*/
...
VOID N3_SetOutChannel_param_0(n123_init_param *pInitParam)
{
	Write_I2C( DEV_ADDR, 0xFF, NULL, BANK1,1);
	Write_I2C( DEV_ADDR, 0xC0, NULL, 0x00,1);
	I2CWriteByte(DEV_ADDR, 0xFF, 0x05);
	...
	if(pInitParam->fmt_res == VDEC_N123_FMT_SH720){
		...
		I2CWriteByte(DEV_ADDR, 0xCB, I2CReadByte(DEV_ADDR,0xCB) | 0x01);
	}else if(pInitParam->fmt_res == VDEC_N123_FMT_720P) {
		I2CWriteByte(DEV_ADDR, 0xCF, 0x43);
		I2CWriteByte(DEV_ADDR, 0xCB, I2CReadByte(DEV_ADDR,0xCB)&0xfe);
	}else if(pInitParam->fmt_res == VDEC_N123_FMT_1080P) {
		I2CWriteByte(DEV_ADDR, 0xCF, 0x76);
		I2CWriteByte(DEV_ADDR, 0xCB, I2CReadByte(DEV_ADDR,0xCB)&0xfe);
	}
	printf("11111111111111111 N3_SetOutChannel_param_0: %x \n",I2CReadByte(DEV_ADDR,0xCB));
	
	I2CWriteByte(DEV_ADDR, 0xFF, 0x00);
	if(pInitParam->fmt_fps == VDEC_N123_FMT_PAL){
		I2CWriteByte(DEV_ADDR, 0x7A, I2CReadByte(DEV_ADDR,0x7A)|0x03);//Cb、Cr change
		I2CWriteByte(DEV_ADDR, 0x7B, 0x33);//default
	}else{
		I2CWriteByte(DEV_ADDR, 0x7A, I2CReadByte(DEV_ADDR,0x7A)&0xfc);//Cb、Cr change
		I2CWriteByte(DEV_ADDR, 0x7B, 0x11);//Cb、Cr change
	}
}
VOID N3_SetOutChannel_param_1(n123_init_param *pInitParam)
{
	Write_I2C( DEV_ADDR, 0xFF, NULL, BANK1,1);
	Write_I2C( DEV_ADDR, 0xC2, NULL, 0x11,1);
	I2CWriteByte(DEV_ADDR, 0xFF, 0x06);
	...
	if(pInitParam->fmt_res == VDEC_N123_FMT_SH720){
		...
		I2CWriteByte(DEV_ADDR, 0xCB, I2CReadByte(DEV_ADDR,0xCB) | 0x04);
	}else if(pInitParam->fmt_res == VDEC_N123_FMT_720P) {
		I2CWriteByte(DEV_ADDR, 0xCD, 0x43);
		I2CWriteByte(DEV_ADDR, 0xCB, I2CReadByte(DEV_ADDR,0xCB)&0xfb);
	}else if(pInitParam->fmt_res == VDEC_N123_FMT_1080P) {
		I2CWriteByte(DEV_ADDR, 0xCD, 0x76);
		I2CWriteByte(DEV_ADDR, 0xCB, I2CReadByte(DEV_ADDR,0xCB)&0xfb);
	}
	printf("11111111111111111 N3_SetOutChannel_param_1: %x \n",I2CReadByte(DEV_ADDR,0xCB));
	I2CWriteByte(DEV_ADDR, 0xFF, 0x00);
	if(pInitParam->fmt_fps == VDEC_N123_FMT_PAL){ //有些制式摄像头,cb,cr是反的
		I2CWriteByte(DEV_ADDR, 0x7A, I2CReadByte(DEV_ADDR,0x7A)|0x30);//Cb、Cr change
		I2CWriteByte(DEV_ADDR, 0x7B, 0x33);//Cb、Cr change
	}else{
		I2CWriteByte(DEV_ADDR, 0x7A, I2CReadByte(DEV_ADDR,0x7A)&0xbf);//default
		I2CWriteByte(DEV_ADDR, 0x7B, 0x11);//default
	}
}
...
int N3_Init( n123_init_param *pInitParam,unsigned char ch )
{
	DEV_ADDR = pInitParam->devaddr;
	...
	else if(pInitParam->fmt_res == VDEC_N123_FMT_720P)
	{
		...
		I2CWriteByte(DEV_ADDR, 0xBC, 0x80);
		I2CWriteByte(DEV_ADDR, 0xBD, 0x80);
		I2CWriteByte(DEV_ADDR, 0xCB, 0x00); //0xaa//hsync and vsync  invert
		#endif
...
u32_t NVP6214_get_video_0_height (void)
{
	return Sensor_Output_0_Height ;
}

u32_t NVP6214_get_video_0_width (void)
{
	return Sensor_Output_0_Width;
}

u32_t NVP6214_get_video_1_height (void)
{
	return Sensor_Output_1_Height;
}

u32_t NVP6214_get_video_1_width (void)
{
	return Sensor_Output_1_Width;
}
...
#define AUDIO_C	 GPIO92 //新增两个声音通道
...
int video_format_Pre_0 = 0; 
static unsigned char Sensor_Output_Format_0 = 0;
static unsigned char Sensor_Pre_Format_0 = 0xff;
extern unsigned int SensorData_CH0_Format;
extern BOOL SensorData_CH0_Status;

VOID N3_VideoFormat_Detection_0(  n123_init_param *pInitParam, int ch )
{
	...
	SensorData_CH0_Status = video_format;
	if(video_format_Pre_0 != video_format)
	{	
		video_format_Pre_0 = video_format;
		return ;
	}
	if(video_format != Sensor_Pre_Output_Format_0)
	{
		Sensor_Pre_Output_Format_0 = video_format;
		if(video_format == SD_NTSC)
		{
	   		Sensor_Output_0_Width = 720;
	   		Sensor_Output_0_Height = 240;
			init_param.fmt_fps = VDEC_N123_FMT_NTSC;
			init_param.fmt_res = VDEC_N123_FMT_SH720;
			N3_Mode=0x01;
            SensorData_CH0_Format = AP_DSTRESOURCE_CVBS_N;
		}
		else if(video_format == SD_PAL)
		{
	   		Sensor_Output_0_Width = 720;
	   		Sensor_Output_0_Height = 288;
		...
		else if(video_format == AHD_30FPS)
		{
	   		Sensor_Output_0_Width = 1280;
	   		Sensor_Output_0_Height = 720;
		...
		else if(video_format == AHD_25FPS)
		{
	   		Sensor_Output_0_Width = 1280;
	   		Sensor_Output_0_Height = 720;
		...
		else if(video_format == AHD_60FPS)
		{
	   		Sensor_Output_0_Width = 1280;
	   		Sensor_Output_0_Height = 720;
		...
		else if(video_format == AHD_50FPS)
		{
	   		Sensor_Output_0_Width = 1280;
	   		Sensor_Output_0_Height = 720;
		...
		else if(video_format == AFHD_30FPS)
		{
	   		Sensor_Output_0_Width = 1920;
	   		Sensor_Output_0_Height = 1080;
		...
		else if(video_format == AFHD_25FPS) //默认1080P 25fps
		{
	   		Sensor_Output_0_Width = 1920;
	   		Sensor_Output_0_Height = 1080;
			init_param.fmt_fps = VDEC_N123_FMT_25P;
		...
		if((video_format != NO_DETECT)&&(video_format != Sensor_Pre_Format_0))
		{
			if(Sensor_Pre_Format_0 != 0xff)
			{
				Notification_Itu601_Isp_Scaler_Stop();
			}
			Sensor_Pre_Format_0 = video_format;
			if(N3_Mode == 0x00) {
				_0_30P_1_25P = init_param.fmt_fps == VDEC_N123_FMT_30P ? 0 : 1 ;
				N3_Init_ch(2,_0_30P_1_25P,ch);
			}else if(N3_Mode == 0x01){
				_0_30P_1_25P = init_param.fmt_fps == VDEC_N123_FMT_NTSC ? 0 : 1 ;
				N3_Init_ch(0,_0_30P_1_25P,ch);	//CH1/2/3/4 720H
			}else if(N3_Mode == 0x04){
				_0_30P_1_25P = init_param.fmt_fps == VDEC_N123_FMT_30P ? 0 : 1 ;
				N3_Init_ch(3,_0_30P_1_25P,ch);	//CH1/2/3/4 1080P
			}
			//N3_SetInputChannel(&init_param,0);
			N3_SetOutChannel_param_0(&init_param);
		    Notification_Itu601_Isp_Scaler_Init();
		}
	}
}

int video_format_Pre_1 = 0;
static unsigned char Sensor_Output_Format_1 = 0;
static unsigned char Sensor_Pre_Format_1 = 0xff;
static unsigned char Sensor_Pre_Output_Format_1=NO_DETECT;
extern unsigned int SensorData_CH1_Format;
extern BOOL SensorData_CH1_Status;

void N3_VideoFormat_Detection_1(	n123_init_param *pInitParam, int ch )
{
	int video_format,video_detect;
	int _0_30P_1_25P;
	DEV_ADDR = pInitParam->devaddr;
	I2CWriteByte(DEV_ADDR, 0xFF, 0x00);
	video_format = I2CReadByte(DEV_ADDR, 0xD0+ch%4);
	SensorData_CH1_Status = video_format;
	//printf("11111 video_format: %x Sensor_Pre_Output_Format_0: %d ch: %d\n",video_format,Sensor_Pre_Output_Format_1,ch);
	if(video_format_Pre_1 != video_format)
	{	
		video_format_Pre_1 = video_format;
		return ;
	}
	if(video_format != Sensor_Pre_Output_Format_1)
	{
		Sensor_Pre_Output_Format_1 = video_format;
		if(video_format == SD_NTSC)
		{
	   		Sensor_Output_1_Width  = 720;
	   		Sensor_Output_1_Height = 240;
			init_param.fmt_fps = VDEC_N123_FMT_NTSC;
			init_param.fmt_res = VDEC_N123_FMT_SH720;
			N3_Mode=0x01;
		...
		{
	   		Sensor_Output_1_Width  = 1280;
	   		Sensor_Output_1_Height = 720;
		...
		{
	   		Sensor_Output_1_Width  = 1920;
	   		Sensor_Output_1_Height = 1080;
		...
		if((video_format != NO_DETECT)&&(video_format != Sensor_Pre_Format_1))
		{
			if((Sensor_Pre_Format_1 != 0xff)||(Sensor_Pre_Format_1 != AP_DSTRESOURCE_1080P_25) || (Sensor_Pre_Format_1 != AP_DSTRESOURCE_1080P_30)) //第一次进入
			{
				DRV7116_PlugOut(); //清除缓存
			}
			Sensor_Pre_Format_1 = video_format;
			if(N3_Mode == 0x00) {
				_0_30P_1_25P = init_param.fmt_fps == VDEC_N123_FMT_30P ? 0 : 1 ;
				N3_Init_ch(2,_0_30P_1_25P,ch);
			}else if(N3_Mode == 0x01){
			...
			#endif
			N3_SetOutChannel_param_1(&init_param);
			if((Sensor_Pre_Format_1 != AP_DSTRESOURCE_1080P_25) || (Sensor_Pre_Format_1 != AP_DSTRESOURCE_1080P_30))
				DRV7116_PlugIn();
		}
	}
}
...
extern BOOL Close_Audio_Sound; //在关机的时候不要进入检测状态
...
void N3_VideoFormat_Detection(  n123_init_param *pInitParam, int ch )
{
    ...
    if(((video_format != Sensor_Pre_Output_Format) || Manual_Switch)&&Close_Audio_Sound)
    {
		...
		else if(Sensor_Output_Format == AFHD_25FPS) //默认1080P 25fps
		{
	   		Sensor_Output_Width = 1920;
	   		Sensor_Output_Height = 1080;
			init_param.fmt_fps = VDEC_N123_FMT_25P;
        ...
        if(Sensor_Output_Format == NO_DETECT) {
            ...
            if(APP_GetVOl_OnOff()&&(ch!=3))
                ...
                HW_Auido_SetLevel(0);
            Delay_NVP6214_Data = 1; //热插拔重新插入需要间隔15针的数据来防止数据不稳
        }
		if((((Sensor_Output_Format != NO_DETECT)&& (Sensor_Output_Format !=Sensor_Pre_Format))  || Manual_Switch)&&Close_Audio_Sound)
		{
            ...
            if(Sensor_Pre_Format != 0xff) {//排除第一次
                if((Sensor_Pre_Format != SD_NTSC) && (Sensor_Pre_Format != SD_PAL))
                {
                    // Itu601
        			Notification_Itu601_Isp_Scaler_Stop(); //清除缓存
                }else {
                    DRV7116_PlugOut();
                }
            }
...
static void nvp6124b_device_init(void)
{
	...
	N3_Init(&init_param,0);
    #if 0
	N3_SetInputChannel(&init_param,1); //0:通道1 //1: 通道2 //2:通道3 //3: 通道4
    N3_SetCH2_VD1();
	msleep(200);
...
{
    OS_SignalEvent(NVP6214B_DET_EVENT, &TCB_NVP6124B_Task); /* 通知事件 */
...
u8_t Select_AHD_Camera(void)
{
    ...
    if(((GetGPIOPadData(VIDEO_ACC4_DET) == 0) ||AHD_Reverse_TimeDelay(AHD4_Delay,AHD4_Reverse_Status))&&AV_Select_AHD){
        ...
        if(GetGPIOPadData(VIDEO_ACC4_DET) == 0) { //只有倒车的时候初始化.
            AHD4_Delay = AP_GetDelay_Camera4();
            AHD4_Delay = AHD4_Delay?AHD4_Delay:1;//倒车延时:转向灯延时
            AHD_Timer_Count = 0;
        }
        AHD_Status = 1;
        Manual_Channel = 1;
    ...
    //AHD_ChannelNum_Data = ACC_DET;//在倒车的时候切换
    ACC_Det_Start = ACC_DET; //倒车需要确认哪个通道
    ACC_Select = ACC_DET;
    if(ACC_DET != ACC_DET_Pre)
    {
        HW_Auido_SetLevel(0); //#20180913## @Edison add //切换声道需要静音
        if(ACC_DET == ACC4_DET) {
            SetGPIOPadData(VIDEO_AHD4,euDataHigh);
            SetGPIOPadData(VIDEO_AHD3,euDataLow);
            SetGPIOPadData(VIDEO_AHD2,euDataLow);
...
static void nvp6124b_task (void)
{
    ...
    while(1)
    {
        nvp6214_event = OS_WaitEvent(NVP6214B_DET_EVENT);

        if(nvp6214_event & NVP6214B_DET_EVENT)
        {
			N3_VideoFormat_Detection_0(&init_param,0);
			N3_VideoFormat_Detection_1(&init_param,1);
