...
FS_FILE *fTiledOutput = NULL;
FS_FILE *fchroma2 = NULL;
...
#define EXPIRY_DATE (u32)0xFFFFFFFF
    ...
    /* expiry stuff */
    {
        ...
        {
            printf("EVALUATION PERIOD EXPIRED.\n"
    ...
    {
#ifdef ASIC_TRACE_SUPPORT
        DEBUG_PRINT(("Usage: %s [-Nn] [-Ooutfile] [-X] [-C] [-R] [-Sfile.hex] [-W] [-L] [-P] [-T] [-I] [-F] [-B] file.h264\n", argv[0]));
#else
        DEBUG_PRINT(("Usage: %s [-Nn] [-Ooutfile] [-X] [-C] [-R] [-Sfile.hex] [-W] [-L] [-P] [-T] [-I] file.h264\n", argv[0]));	
#endif	
     ...
     * fails -> exit */
    finput = FS_FOpen(argv[argc - 1], "rb");
    if(finput == NULL)
    {
        DEBUG_PRINT(("UNABLE TO OPEN INPUT FS_FILE\n"));
        return -1;
    }
    ...
    if(argc < 3)
    {
        DEBUG_PRINT(("Usage: %s [-Nn] [-X] [-R] [-W] [-L] [-F] [-B] [-I] file.h264  pp.cfg\n",
                     argv[0]));
        DEBUG_PRINT(("\t-Nn forces decoding to stop after n pictures\n"));
        DEBUG_PRINT(("\t-X Disable output file writing\n"));
        DEBUG_PRINT(("\t-R disable DPB output reordering\n"));
    ...
    /* open data file */
    finput = FS_FOpen(argv[argc - 2], "rb");
    if(finput == NULL)
    {
    ...
    if(saveIndex == 1)
    {
        findex = FS_FOpen("stream.cfg", "w");
        if(findex == NULL)
        {
            DEBUG_PRINT(("UNABLE TO OPEN INDEX FS_FILE\n"));
            return -1;
        }
    } 
    /* try open index file */
    ...
    fTBCfg = FS_FOpen("tb.cfg", "r");
    if(fTBCfg == NULL)
    {
        DEBUG_PRINT(("UNABLE TO OPEN INPUT FS_FILE: \"tb.cfg\"\n"));
...
/*#if MD5SUM
    ...
    if(!tmp)
    {
        DEBUG_PRINT(("UNABLE TO OPEN TRACE FS_FILE(S)\n"));
    }
    if(nalUnitStream)
    ...
    FS_FSeek(finput, 0L, FS_SEEK_END);
    strmLen = FS_FTell(finput);
    //rewind(finput);
	 FS_FSeek (finput, 0, SEEK_SET);

	 // 选择 longStream == 0，forceWholeStream = 0
	 // packetize 需配置为1，逐帧解码
	 packetize = 1;
    ...
    if(!longStream)
    {
        ...
        FS_FRead(byteStrmStart, 1, strmLen, finput);
    ...
    if(longStream && !packetize && !nalUnitStream)
    {
        if(useIndex == 1)
        {
            ...
            /* read index */
            fscanf(findex, "%d", &nextIndex);
            amount = nextIndex - curIndex;
            curIndex = nextIndex;

            /* read first block */
            decInput.dataLen = FS_FRead(decInput.pStream, 1, amount + 8, finput);
        }
        else
        {
            decInput.dataLen =
                FS_FRead(decInput.pStream, 1, DEC_X170_MAX_STREAM, finput);
        }
        ...
        case H264DEC_HDRS_RDY:
            {
                ...
                hdrsRdy = 1;
                TBSetRefbuMemModel( &tbCfg, &((decContainer_t *) decInst)->refBufferCtrl );
                ...
                H264DecPeek(decInst, &decPicture) == H264DEC_PIC_RDY)
            {
                ...
                else
                {
                    if (lastField)
                        WriteOutput(outFileName, outFileNameTiled,
                                    ...
                                    decInfo.picWidth,
                                    decInfo.picHeight,
                                    picDecodeNumber - 2, decInfo.monoChrome);
                    lastField = 0;
                    memcpy(tmpImage, imageData, picSize);
                }
                ...
                if (!lastField)
                    WriteOutput(outFileName, outFileNameTiled,
                                ...
                                decInfo.picWidth,
                                decInfo.picHeight,
                                picDecodeNumber - 1, decInfo.monoChrome);
        ...
        if(longStream && !packetize && !nalUnitStream)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if(FS_FSeek(finput, -decOutput.dataLeft, SEEK_CUR) == -1)
                    {
    ...
    if(saveIndex == 1)
    {
        fprintf(findex, "%d\n", FS_FTell(finput) - decOutput.dataLeft);
    }

    if(useIndex == 1 || saveIndex == 1)
    {
        FS_FClose(findex);
    }
    ...
    foutput = FS_FOpen(outFileName, "rb");
    if(NULL == foutput)
    {
        strmLen = 0;
    }
    ...
    {
        FS_FSeek(foutput, 0L, FS_SEEK_END);
        strmLen = (u32) FS_FTell(foutput);
        FS_FClose(foutput);
    }
                 ...
                 u32 picWidth, u32 picHeight, u32 frameNumber, u32 monoChrome)
{
    ...
    if(foutput == NULL)
    {
        ...
        {
            foutput = FS_FOpen(filename, "wb");
            if(foutput == NULL)
            {
                DEBUG_PRINT(("UNABLE TO OPEN OUTPUT FS_FILE\n"));
                return;
            }
        }
#ifdef _ENABLE_2ND_CHROMA
        ...
        {
            fchroma2 = FS_FOpen("out_ch8pix.yuv", "wb");
            if(fchroma2 == NULL)
            {
                DEBUG_PRINT(("UNABLE TO OPEN OUTPUT FS_FILE\n"));
                return;
    ...
    if(fTiledOutput == NULL && tiledOutput)
    {
        ...
        {
            fTiledOutput = FS_FOpen(filenameTiled, "wb");
            if(fTiledOutput == NULL)
            {
                DEBUG_PRINT(("UNABLE TO OPEN TILED OUTPUT FS_FILE\n"));
                return;
            }
        }
    }
...
#ifdef MD5SUM
    TBWriteFrameMD5Sum(foutput, data, picSize, frameNumber);
    ...
    {
        tmp = picSize * 2 / 3;
        FS_FWrite(data, 1, tmp, foutput);
        for(i = 0; i < tmp / 4; i++)
            FS_FWrite(data + tmp + i * 2, 1, 1, foutput);
        for(i = 0; i < tmp / 4; i++)
            FS_FWrite(data + tmp + 1 + i * 2, 1, 1, foutput);
    }
    else    /* semi-planar */
    {
        FS_FWrite(data, 1, picSize, foutput);
        if(monoChrome)
        {
            FS_FWrite(grey_chroma, 1, grey_chroma_size, foutput);
        }
    }
...
#else
        FS_FWrite(pCh, 1, picSize/3, fchroma2);
#endif
    }
#endif
...
u32 NextPacketFromFile(u8 ** pStrm)
{
    ...
    {
        DEBUG_PRINT(("FS_FILE POSITION GET ERROR\n"));
        return 0;
    }
    ...
    if(useIndex == 0)
    {
        ...
        {
            DEBUG_PRINT(("FS_FILE POSITION SET ERROR\n"));
            return 0;
        }

        /* Read the rewind stream */
        FS_FRead(*pStrm, 1, index, finput);
        if(feof(finput))
        {
    ...
    else
    {
        ...
        fPos = FS_FTell(finput);
        /*DEBUG_PRINT(("fPos: %u\n", fPos));
        DEBUG_PRINT(("curIndex: %u\n", curIndex));*/
        if(fPos != curIndex)
        {
            FS_FSeek(finput, curIndex - fPos, SEEK_CUR);
            /*DEBUG_PRINT(("Seek %d\n", curIndex - fPos));*/
        }

        fscanf(findex, "%u", &nextIndex);
        /*DEBUG_PRINT(("nextIndex: %u\n", nextIndex));*/
        amount = nextIndex - curIndex;
        /*DEBUG_PRINT(("amount: %u\n", amount));*/
        curIndex = nextIndex;
        FS_FRead(*pStrm, 1, amount, finput);
        index = amount;
    }
    /* If we skip this packet */
    if(picRdy && nextPacket &&
...
u32 fillBuffer(u8 *stream)
{
    ...
    if(curIndex != FS_FTell(finput))
    {
        DEBUG_PRINT(("SEEK\n"));
        FS_FSeek(finput, curIndex, SEEK_SET);
    }
    ...
    /* read data */
    dataLen = FS_FRead(stream, 1, amount, finput);
