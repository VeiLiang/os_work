...
#ifdef H264_DECODE_DEBUG
#include <stdio.h>
extern unsigned int h264decasic_time;
#endif
/*------------------------------------------------------------------------------
                         ...
                         H264DecOutput * pOutput)
{
    ...
    dma_flush_range ((unsigned int)pInput->pStream, pInput->dataLen + (unsigned int)pInput->pStream);
...
#ifdef H264_DECODE_DEBUG
						  h264decasic_time += XM_GetTickCount() - ticket;
                              ...
                              H264DecPicture * pOutput, u32 endOfStream)
{
    ...
    if(outPic != NULL)
    {
        ...
        if(pDecCont->pp.ppInstance != NULL && (pDecCont->pp.ppInfo.multiBuffer != 0))
        {
		  ...
		  // 20180719 私有数据解析
		  // 检查是否存在私有数据。userData[0]表示私有数据的字节总长度(包括总长度本身)
		  if( pDecCont->user_data[0] )
    {
			  memcpy (pOutput->userData, pDecCont->user_data, pDecCont->user_data[0]);
			  pDecCont->user_data[0] = 0;
    }
		  else
		  {
			  pOutput->userData[0] = 0;
		  }
