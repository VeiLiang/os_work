...
// 20180719 zhuoyonghong 加入帧私有数据访问机制
// SEI解析
static void extract_user_data_from_sei (decContainer_t * pDecCont, strmData_t *pStrmData)
{
	u32 tmp, i, value;
	u32 cnt;
	u8* user_data;
	do
	{
		...
		if (tmp == END_OF_STREAM)
			return;
		if(tmp == SEI_BUFFERING_PERIOD || tmp == SEI_PIC_TIMING)
		{
			i = h264bsdGetBits(pStrmData, 8);
			while(i > 0)
			{
				h264bsdGetBits(pStrmData, 8);
				i --;
			}
		}
		else if(tmp == SEI_USER_DATA_UNREGISTERED)
		{
			memset (pDecCont->user_data, 0, sizeof(pDecCont->user_data));
			cnt = 0;
			do
			{
				tmp = h264bsdGetBits(pStrmData, 8);	
			...
			cnt += tmp;
			user_data = (u8 *)pDecCont->user_data;
			while(cnt > 0)
			{
				tmp = h264bsdGetBits(pStrmData, 8);
				if (tmp == END_OF_STREAM)
		...
		else if(tmp == SEI_FILLER_PAYLOAD)
		{
			cnt = 0;
			do
			{
...
/* Code */
    ...
    if(!picReady)
    {
        ...
        case NAL_CODED_SLICE:
            ...
            /* TODO pit?sk?siirt?? pelk?st??n rlc moodille, note that
        ...
        case NAL_SEI:
            ...
            ret = H264BSD_RDY;
				// 20180719 加入帧私有数据访问机制
				extract_user_data_from_sei (pDecCont, &strm);
            goto NEXT_NAL;
