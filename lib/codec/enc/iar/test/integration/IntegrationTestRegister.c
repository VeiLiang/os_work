...
int on2_integration_main(int argc, char *argv[])
{
    ...
    if(!ewlInst)
    {
        printf("Failed to initialize EWL\n");
        return -1;
    }
    ...
    if(ret != EWL_OK)
    {
        printf("Failed to release the instance. Error code: %8i\n",
                ret);
    }
...
void TestRegister(void)
{
    ...
    if(ret != EWL_OK)
    {
        printf("\tFAILED to receive interrupt. Error code: %8i\n",
                ret);
    }
    else
    {
    ...
    printf("ASIC Integration register test bit 2\n");
    printf("\tCopying %i bytes of data\n", DATA_LENGTH * 8);
    ...
    for(i = DATA_LENGTH * 2; i < BUFFER_SIZE * 2; i++)
    {
        ...
        {
            printf("\tBuffer changed at 0x%08x [0x%08x]\n",
