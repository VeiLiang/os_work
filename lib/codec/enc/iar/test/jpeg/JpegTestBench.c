...
int jpeg_encode_main(int argc, char *argv[])
{
  ...
  //  printf(
	...
	/* Handle error situation */
     ...
     //   fout = FS_FOpen(cmdl.output, "wb");
		  printf("filename:%s\n",filename);
		  fout = FS_FOpen( filename , "wb");
		//  strcpy(filename , cmdl.output);//add
        if(fout == NULL)
        {
            ...
            do
            {
                ...
                if(JpegReadPic
                   ((u8 *) pictureMem.virtualAddress,
           		 ...
           		 printf("JpegEncode read one frame cost %d ms\n", tickscount);
					 
				    tickscount = XM_GetTickCount();
                ret = JpegEncEncode(encoder, &encIn, &encOut);
					 tickscount = XM_GetTickCount() - tickscount ;
           		 printf("JpegEncode one frame cost %d ms\n", tickscount);
                switch (ret)
                {
                case JPEGENC_RESTART_INTERVAL:
                    ...
                    if(writeOutput)
                    {
							  FS_FClose(fout);
		//					  memset(filename,0,256);
		//					  sprintf(filename,"%02d.jpg",picCnt);
							  printf("writeOutput filename:%s\n", filename );
							  fout = FS_FOpen(filename, "wb");
							  if(!fout)
