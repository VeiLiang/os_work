...
//static char input[] = "\\MINITEST\\RAW\\VIDEO\\TEST.YUV";	
	static char input[] = "\\MINITEST\\RAW\\VIDEO\\19201088.YUV";	
//static char output[] = "\\stream.264";
static char output[] = "\\19201088.264";	
//static char output[] = "\\test.264";
...
static FS_FILE *yuvFile = NULL;
static char *yuvFileName = NULL;
static FS_FILE *yuvFileMvc = NULL;
...
static void WriteStrm(FS_FILE * fout, u32 * outbuf, u32 size, u32 endian);
...
static void WriteNalSizesToFile(const char *file, const u32 * pNaluSizeBuf,
                                u32 buffSize);
static void WriteMotionVectors(FS_FILE *file, i8 *mvs, i32 frame,
...
------------------------------------------------------------------------------*/
int h264_codec_main(int argc, char *argv[])
{
    H264EncInst encoder;
    commandLine_s cmdl;
    ...
    printf("Evaluation version with %d frame limit.\n\n", EVALUATION_LIMIT);
#endif
    ...
    yuvFile = FS_FOpen(cmdl.input, "rb");
    ...
    if (cmdl.inputMvc)
    {
        yuvFile = FS_FOpen(cmdl.inputMvc, "rb");
        ...
        if (cmdl.viewMode < H264ENC_MVC_STEREO_INTER_VIEW_PRED)
            printf("Warning: MVC input with H.264 view mode! Check --viewMode!\n");
    }
...
#define _CODEC_SOFT_RESET_	// 并发测试时关闭, 否则会干扰其他线程
...
i32 Encode(i32 argc, char **argv, H264EncInst encoder, commandLine_s * cml)
{
    ...
    FS_FILE *fmv = NULL;
    u8 *pUserData;
	 unsigned int r_start_ticket, w_start_ticket, e_start_ticket;
	 unsigned int r_ticket, w_ticket, e_ticket;
    ...
    if (cml->inputMvc)
	 ...
	 e_start_ticket = XM_GetTickCount ();
    /* Start stream */
    ...
    if(ret != H264ENC_OK)
    {
	 ...
	 e_ticket += XM_GetTickCount() - e_start_ticket;
    ...
    if (cml->mvOutput)
    {
        fmv = FS_FOpen("mv.txt", "wb");
        if(fmv == NULL)
        {
            printf("Failed to create mv.txt output file.\n");
            return -1;
        }
    }
    ...
    if(cml->byteStream == 0)
    {
	 ...
	 w_ticket += XM_GetTickCount () - w_start_ticket;
  ...
  nextinput:
    while(1)
    {
		 r_start_ticket = XM_GetTickCount();
    ...
    if(frameCnt >= EVALUATION_LIMIT)
...
#endif
		  r_ticket += XM_GetTickCount () - r_start_ticket;
        ...
        case H264ENC_FRAME_READY:
            ...
            if (pUserData)
            {
				...
				/*
				// write constructed frame data
				{
					static int con_index = 0;
					unsigned int reg_mirror[128];
					char con_name[32];
					h264Instance_s* inst = (h264Instance_s*)encoder;
					sprintf (con_name, "\\con_%03d.yuv", con_index);
					con_index  ++;
					memset (reg_mirror, 0, sizeof(reg_mirror));
					EncAsicGetRegisterValue (inst->asic.ewl, &reg_mirror, HEncBaseRecLum);
					EncAsicGetRegisterValue (inst->asic.ewl, &reg_mirror, HEncBaseRecChr);
					printf ("HEncBaseRecLum=0x%08x\n", reg_mirror[9]);
					printf ("HEncBaseRecChr=0x%08x\n", reg_mirror[10]);	
					FS_FILE *fp = FS_FOpen (con_name, "wb");
					if(fp)
					{
						FS_FWrite ((char *)reg_mirror[9], 1, 352*288, fp);
						FS_FWrite ((char *)reg_mirror[10], 1, 352*288/2, fp);
						FS_FClose (fp);
					}
    ...
    if(yuvFileMvc != NULL)
        FS_FClose(yuvFileMvc);
	 
	 w_ticket += XM_GetTickCount() - w_start_ticket;
	 
	 printf ("\ncodec=%d, encode=%d\n", codeFrameTicket / codedFrameCnt,
				e_ticket / codedFrameCnt);
	 
	 printf ("codec=%d, encode=%d, read=%d, write=%d, total=%d\n", 
				codeFrameTicket / codedFrameCnt,
				e_ticket / codedFrameCnt, r_ticket / codedFrameCnt, w_ticket / codedFrameCnt, 
				(e_ticket + r_ticket + w_ticket) / codedFrameCnt);
...
int AllocRes(commandLine_s * cmdl, H264EncInst enc)
{
    ...
    outbufSize = 1 * pictureMem.size < (1024 * 1024 * 8) ?
        1 * pictureMem.size : (1024 * 1024 * 8);
            ...
            i32 format)
{
    ...
    if((name != yuvFileName) && (yuvFileMvc == NULL))
    {
        ...
        FS_FSeek(yuvFileMvc, 0, SEEK_END);
        file_size = FS_FTell(yuvFileMvc);
    }
    ...
    if(FS_FSeek(readFile, (off_t)size * nro, SEEK_SET) != 0)
    {
...
u8* ReadUserData(H264EncInst encoder, char *name)
{
    ...
    FS_FSeek(file, 0L, SEEK_END);
    byteCnt = FS_FTell(file);
    //rewind(file);
    ...
    FS_FRead(data, sizeof(u8), byteCnt, file);
    FS_FClose(file);
...
------------------------------------------------------------------------------*/
void WriteStrm(FS_FILE * fout, u32 * strmbuf, u32 size, u32 endian)
{
    ...
    int ret = FS_FWrite(strmbuf, 1, size, fout);
	 printf ("FS_FWrite handle=%08x, size = %d, ret = %d\n",  fout, size, ret);
}
...
------------------------------------------------------------------------------*/
void WriteMotionVectors(FS_FILE *file, i8 *mvs, i32 frame, i32 width, i32 height)
{
	 char msg[128];
    i32 i;
    ...
    {
        sprintf(msg, "  %3d,%3d", mvs[i*4], mvs[i*4+1]);
		  FS_Write (file, msg, strlen(msg));
    ...
    for (i = 0; i < mbPerFrame; i++)
    {
        sprintf(msg, " %5d", (u32)(sads[i*4+2] << 8) | sads[i*4+3]);
		  FS_Write (file, msg, strlen(msg));
        if ((i % mbPerRow) == mbPerRow-1)
            // fprintf(file, "\n");
			  FS_Write (file, "\n", strlen("\n"));
