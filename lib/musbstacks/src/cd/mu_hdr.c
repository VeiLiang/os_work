						  ...
						  uint8_t bBind, uint8_t* pbIsTx)
{
 ...
 typedef struct stMGC_USB_Event
{
	void* pParam;
...
#define USB_DEDIGHERING_STATUS_IDLE				0
#define USB_DEDIGHERING_STATUS_RUNNING			1
#define USB_DEDIGHERING_STATUS_PASS				2

static HardwareTimer *lg_pUsb_DetheringTimer = NULL;
static UINT32 lg_ulUsbDeDitheringStatus = USB_DEDIGHERING_STATUS_IDLE;
static MGC_USB_Event Usb_Event;

unsigned char lg_Usb_Present = 0;

extern uint8_t MGC_PostIsrEvent (MGC_Controller* pControllerImpl);
static void MGC_Usb_Dedithering_Handler(void *pdata, uint16_t timeout)
{
	MGC_USB_Event* pUsb_Event = (MGC_USB_Event*)pdata;
	MUSB_Controller* pController = (MUSB_Controller*)pUsb_Event->pParam;
	MGC_Controller* pControllerImpl = (MGC_Controller*)pController->pPrivateData;
	int isrValue;
	
	StopHardwareTimer(lg_pUsb_DetheringTimer);
	lg_ulUsbDeDitheringStatus = USB_DEDIGHERING_STATUS_PASS;

	MUSB_PRINTK("Usb Insert\r\n");
	lg_Usb_Present = 1;
	isrValue = MGC_DrcIsr(pControllerImpl, pUsb_Event->bIntrUsbValue, pUsb_Event->wIntrTxValue, pUsb_Event->wIntrRxValue);
	// 20140221 ZhuoYongHong
	// 需要触发USB ISR
	/* wake BSR task if indicated */
	if(isrValue > 0)
	{
		MGC_PostIsrEvent (pControllerImpl);
	}
	
}

void MGC_InitUsbDeditheringTimer(void* pParam)
{
	if(!lg_pUsb_DetheringTimer)
	{
		lg_pUsb_DetheringTimer = NewHardwareTimer(100, 0, MGC_Usb_Dedithering_Handler);
		if(lg_pUsb_DetheringTimer)
			AddHardwareTimer(lg_pUsb_DetheringTimer);

		Usb_Event.pParam = pParam;
		lg_pUsb_DetheringTimer->pdata = &Usb_Event;
		lg_ulUsbDeDitheringStatus = USB_DEDIGHERING_STATUS_IDLE;
	}
}
...
int MGC_HdrcIsr(void* pParam)
{
	...
	MUSB_PRINTK("MGC_HdrcIsr INTRUSB 0x%x INTRTX 0x%x INTRRX 0x%x\n", bIntrUsbValue, wIntrTxValue, wIntrRxValue);
	...
	{
		printk("usb babble detect \n");		
	}
	/* call common ISR */
	//这里可以对connect  和disconnect  进行去抖处理
	// 20140221 zhuoyonghong
	// wake BSR task if indicated
	result = 0;
	if(bIntrUsbValue & MGC_M_INTR_CONNECT)
	{
		Usb_Event.bIntrUsbValue = bIntrUsbValue;
		Usb_Event.wIntrTxValue = wIntrTxValue;
		Usb_Event.wIntrRxValue = wIntrRxValue;
		//usb insert
		printk("usb insert \n");
		switch(lg_ulUsbDeDitheringStatus)
		{
			case USB_DEDIGHERING_STATUS_IDLE:
				if(lg_pUsb_DetheringTimer)
					StartHardwareTimer(lg_pUsb_DetheringTimer);
				lg_ulUsbDeDitheringStatus = USB_DEDIGHERING_STATUS_RUNNING;
				// 20140221 zhuoyonghong
				// wake BSR task if indicated
				result = 1;
				break;
				
			case USB_DEDIGHERING_STATUS_RUNNING:
				if(lg_pUsb_DetheringTimer)
					StartHardwareTimer(lg_pUsb_DetheringTimer);
				// 20140221 zhuoyonghong
				// wake BSR task if indicated
			...
			case USB_DEDIGHERING_STATUS_PASS:
	...
	else if(bIntrUsbValue & MGC_M_INTR_DISCONNECT)
	{
		...
		switch(lg_ulUsbDeDitheringStatus)
		{
			...
			case USB_DEDIGHERING_STATUS_PASS:
				MUSB_PRINTK("Usb Remove\r\n");
				lg_Usb_Present = 0;
				MGC_DrcIsr(pControllerImpl, bIntrUsbValue, wIntrTxValue, wIntrRxValue);
				lg_ulUsbDeDitheringStatus = USB_DEDIGHERING_STATUS_IDLE;
				// 20140221 zhuoyonghong
				// wake BSR task if indicated
				result = 1;
...
uint32_t MGC_HdrcStart(MGC_Controller* pController)
{
	...
	MUSB_PRINTK("====MGC_O_HDRC_INTRUSBE is 0x%x====\n", MGC_Read8(pBase, MGC_O_HDRC_INTRUSBE));
...
uint32_t MGC_HdrcProgramBusState(MGC_Port* pPort)
{
	...
	uint32_t Count = 100;	
	//MUSB_PRINTK("====MGC_HdrcProgramBusState====power is 0x%x devctl is 0x%x bFuncAddr is 0x%x\n", power, devctl, bFuncAddr);

	/* reset? */
	if(pPort->bWantReset)
	{	
		//MUSB_PRINTK("MGC_HdrcProgramBusState pos 0\n");
		nPower |= MGC_M_POWER_RESET;
	}
	else
	{
		//MUSB_PRINTK("MGC_HdrcProgramBusState pos 1\n");
		nPower &= ~MGC_M_POWER_RESET;
	}
	...
	{
		//MUSB_PRINTK("MGC_HdrcProgramBusState pos 2\n");
		nPower |= MGC_M_POWER_RESUME;
	}
	else
	{
		//MUSB_PRINTK("MGC_HdrcProgramBusState pos 3\n");
		nPower &= ~MGC_M_POWER_RESUME;
	}
	...
	if(pPort->bWantSuspend)
	{
		if(pPort->bIsHost)
		{
			//MUSB_PRINTK("MGC_HdrcProgramBusState pos 4\n");
			nPower |= MGC_M_POWER_SUSPENDM;
		}
		else
		{
			//MUSB_PRINTK("MGC_HdrcProgramBusState pos 5\n");
			nPower |= MGC_M_POWER_ENSUSPEND;
		}
	}
	else
	{
		//MUSB_PRINTK("MGC_HdrcProgramBusState pos 6\n");
		nPower &= ~MGC_M_POWER_SUSPENDM;
		nPower &= ~MGC_M_POWER_ENSUSPEND;
	}
	...
	{
		//MUSB_PRINTK("MGC_HdrcProgramBusState pos 7\n");
		nPower |= MGC_M_POWER_HSENAB;
	}
	else
	{
		//MUSB_PRINTK("MGC_HdrcProgramBusState pos 8\n");
		nPower &= ~MGC_M_POWER_HSENAB;
	}
	...
	{
		//MUSB_PRINTK("MGC_HdrcProgramBusState pos 9\n");
		nDevCtl |= MGC_M_DEVCTL_SESSION;
	}
	else
	{
		//MUSB_PRINTK("MGC_HdrcProgramBusState pos 10\n");
		//nDevCtl &= ~MGC_M_DEVCTL_SESSION;
	}
	...
	if(pPort->bWantHost)
	{
		//MUSB_PRINTK("MGC_HdrcProgramBusState pos 11\n");
		nDevCtl |= MGC_M_DEVCTL_HR;
	}
	else
	{
		//MUSB_PRINTK("MGC_HdrcProgramBusState pos 12\n");
		nDevCtl &= ~MGC_M_DEVCTL_HR;
	}
	...
	if(nDevCtl != devctl)
	{
		...
		while(1)
		{
			if(pPort->bWantSession)
			{
				...
				{
					MUSB_PRINTK("MGC_HdrcProgramBusState nDevCtl is 0x%x\n", nDevCtl);
				}

				Count--;
				if(!Count)
				{
					MUSB_PRINTK("MUSB The DevCtl Maybe B-Device nDevCtl is 0x%x\n", nDevCtl);
					break;
				}
			}
			else
	...
	if(pEnd && bBind)
	{
		...
		else
		{
			...
			if(bEnd)
			{
				...
				MUSB_PRINTK("===MGC_HdrcBindEndpoint===MGC_O_HDRC_RXMAXP is 0x%x\n", pEnd->wRxPacketSize);
				if(pPort->bIsHost)
				{
					csr = 0;
				}
			 ...
			 void* pIrp, uint8_t bAllowDma)
{
	...
	/* normal case: set up for packet size */
	MGC_WriteCsr16(pBase, MGC_O_HDRC_RXMAXP, bEnd, pEnd->wRxPacketSize);
	MUSB_PRINTK("===MGC_HdrcStartRx===MGC_O_HDRC_RXMAXP is 0x%x\n", pEnd->wRxPacketSize);
#if 0
	}
#endif
...
#endif
					bResult = MGC_PipePacketReceived(pPort, MUSB_STATUS_OK, pEnd, wRxCount, FALSE, pIrp);
...
uint8_t MGC_HdrcServiceDefaultEnd(MGC_Port* pPort, MGC_BsrItem* pItem)
{
	...
	if(pPort->bIsHost)
	{
		...
		else if(wCsrVal & MGC_M_CSR0_H_ERROR)
		{
			...
			MUSB_PRINTK("Endpoint 0: no response (error)\n");
		...
		else if(wCsrVal & MGC_M_CSR0_H_NAKTIMEOUT)
		{
			...
			MUSB_PRINTK("Endpoint 0: no response (NAK timeout)\n");
			...
			*/
			...
			if(pEnd && pEnd->pRxIrp)
			{
					...
					((pEnd->bBsrResidual != pEnd->bIsrResidual) || (wVal & MGC_M_RXCSR_RXPKTRDY)))
				{
					...
					/* read FIFO count and see if current IRP is satisfied */
					wRxCount = MGC_ReadCsr16(pBase, MGC_O_HDRC_RXCOUNT, bEnd);
					bResult = MGC_PipePacketReceived(pPort, status, pEnd, wRxCount, FALSE, pEnd->pRxIrp);
					/* re-read CSR */
					wVal = MGC_ReadCsr16(pBase, MGC_O_HDRC_RXCSR, bEnd);
					/* clear recv condition if necessary */
...
uint8_t MGC_HdrcLoadFifo(MGC_Port* pPort, uint8_t bEnd, uint32_t dwCount, const uint8_t* pSource)
{
	...
	if(!dwCount)	
		return 0;
	//else
		//MUSB_PRINTK("==MGC_Hdrc Fifo Write==dwCount is 0x%x\n", dwCount);
	...
	for(dwIndex = dwIndex32 = 0; dwIndex32 < dwCount32; dwIndex32++, dwIndex += 4)
	{
		...
		//MUSB_PRINTK("0x%08x ", dwDatum);
	}
	while(dwIndex < dwCount)
	{
		//MUSB_PRINTK("0x%08x ", pSource[dwIndex]);   
...
uint8_t MGC_HdrcUnloadFifo(MGC_Port* pPort, uint8_t bEnd, uint32_t dwCount, uint8_t* pDest)
{
	...
	if(!dwCount)	
		return 0;
	//else
		//MUSB_PRINTK("==MGC_Hdrc Fifo Read==dwCount is 0x%x\n", dwCount);

	/* doublewords when possible */
	for(dwIndex = dwIndex32 = 0; dwIndex32 < dwCount32; dwIndex32++, dwIndex += 4)
	{
		*((uint32_t*)&(pDest[dwIndex])) = MGC_Read32(pBase, bFifoOffset);
		//MUSB_PRINTK("0x%08x ", *((uint32_t*)&(pDest[dwIndex])));
	}
	while(dwIndex < dwCount)
	{
		pDest[dwIndex++] = MGC_Read8(pBase, bFifoOffset);
		//MUSB_PRINTK("0x%08x ", pDest[dwIndex-1]);
	}
	//MUSB_PRINTK("\n");
    ...
    void* pPrivateData, uint8_t bLocalEnd, uint8_t bTransmit)
{
	...
	else
	{
		...
		bQueue = MGC_PipePacketReceived(pPort, status, pEnd, wRxCount, FALSE, pEnd->pRxIrp);
		/* clear recv condition if necessary */
		if((wRxCount < pEnd->wRxPacketSize) || !(wVal & MGC_M_RXCSR_AUTOCLEAR))
		{
			wVal &= ~MGC_M_RXCSR_RXPKTRDY;
