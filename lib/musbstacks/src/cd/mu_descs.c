				   ...
				   uint8_t bDescriptorType, uint8_t bIndex)
{
	...
	if(pHeader && ((pConfig->bDescriptorType == MUSB_DT_CONFIG) || 
...
{
	return (MUSB_EndpointDescriptor*)MUSB_FindDescriptorInBlock(pConfig, (uint8_t*)pInterfaceDescriptor, 
    ...
    uint8_t bIndex)
{
	...
	(pConfig->bDescriptorType == MUSB_DT_OTHER_SPEED)) && pDescriptorBlock)
	{
		...
		while((wOffset < wBufferLength) && (pHeader->bDescriptorType != bLimitType) && (wFoundIndex < (int16_t)bIndex))
		{
			pHeader = (MUSB_DescriptorHeader*)(pDescriptorBlock + wOffset);
			if(pHeader->bLength < 1)
			{
		...
		/* also the tail case */
		if((wOffset == wBufferLength) && (wFoundIndex == (uint16_t)bIndex) && (pHeader->bDescriptorType == bDescriptorType))
		{
			return (uint8_t*)pHeader;
		}
	}
