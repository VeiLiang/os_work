...
uint8_t MGC_HostDestroy(MGC_Port* pPort)
{
	...
	for(wIndex = 0; wIndex < wCount; wIndex++)
	{
		...
		// 20131204 ZhuoYongHong
		// 检查当前设备是否是端口的root device。若是，同时将root device置0
		if(pPort->bOtgState == MUSB_A_HOST && pDevice == pPort->pRootDevice)
			...
			MUSB_EndpointResource* pEndpointResource)
{
	...
	if(pPipe)
	{
		...
		MUSB_PRINTK("====MUSB_OpenPipe====bEndpointAddress0 is 0x%x====\n", pRemoteEnd->UsbDescriptor.bEndpointAddress);
...
uint32_t MGC_HostSetMaxPower(MGC_Port* pPort, uint16_t wPower)
{
	...
	// fix Warning[Pe940]: missing return statement at end of non-void function "MGC_HostSetMaxPower" 
	return 0;
}
