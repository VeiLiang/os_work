...
static void MGC_HsDmaReleaseChannel(MUSB_DmaChannel* pChannel)
{
	...
	MGC_Write16(pBase, MGC_HSDMA_CHANNEL_OFFSET(pImplChannel->bIndex, MGC_O_HSDMA_CONTROL), 0);
				       ...
				       const uint8_t* pBuffer, uint32_t dwLength)
{
	...
	MGC_Write16(pBase, MGC_HSDMA_CHANNEL_OFFSET(bChannel, MGC_O_HSDMA_CONTROL), wCsr);
	...
	MUSB_DmaChannel* pChannel)
{
	...
	uint8_t bChannel = pImplChannel->bIndex;
	uint16_t wCsr = MGC_Read16(pBase, MGC_HSDMA_CHANNEL_OFFSET(bChannel, MGC_O_HSDMA_CONTROL));
...
static uint8_t MGC_HsDmaControllerIsr(void* pPrivateData)
{
	...
	for(bChannel = 0; bChannel < MUSB_HSDMA_CHANNELS; bChannel++)
	{
		...
		{
			pImplChannel = (MGC_HsDmaChannel*)&(pController->aChannel[bChannel]);
			wCsr = MGC_Read16(pBase, MGC_HSDMA_CHANNEL_OFFSET(bChannel, MGC_O_HSDMA_CONTROL));
			dwCount = MGC_Read32(pBase, MGC_HSDMA_CHANNEL_OFFSET(bChannel, MGC_O_HSDMA_COUNT));
...
#if defined(MUSB_DMA) && defined(MUSB_HSDMA) && (MUSB_HSDMA_CHANNELS > 0)
	MGC_HsDmaController* pController = (MGC_HsDmaController*)MUSB_MemAlloc(sizeof(MGC_HsDmaController));
	if(pController)
	{
