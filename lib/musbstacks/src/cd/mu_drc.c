 ...
 */
int MGC_DrcIsr(MGC_Controller* pController, uint8_t bIntrUsbValue, uint16_t wIntrTxValue, uint16_t wIntrRxValue)
{
	MGC_BsrItem qItem;
	uint32_t dwRegVal;
	uint8_t bQueue;
	...
	MGC_DIAG2(3, pController, "IntrTx=", wIntrTxValue, " / IntrRx=", wIntrRxValue, 16, 4);
 ...
 */
uint8_t MGC_DrcServiceDefaultEnd(MGC_Port* pPort, MGC_BsrItem* pItem, uint16_t* pwCsr, uint16_t wCount)
{
...
uint32_t MGC_DrcResetPort(MGC_Port* pPort)
{
	...
	/* kill timer */
	pController->pSystemServices->pfCancelTimer(pController->pSystemServices->pPrivateData, 0);
	...
	/* flush BSR queue */
	pController->pSystemServices->pfFlushBackgroundQueue(pController->pSystemServices->pPrivateData);
...
static void MGC_HostResetStart(void* pParam, uint16_t wTimer)
{
	...
	pController->pSystemServices->pfArmTimer(pController->pSystemServices->pPrivateData, 0, 60, FALSE, MGC_HostResetComplete);
}   /* MGC_HostResetStart */
...
static void MGC_BSrpFailed(void* pParam, uint16_t wTimer)
{
	...
	if((MUSB_B_SRP_INIT == pPort->bOtgState) || (MUSB_AB_IDLE == pPort->bOtgState))
	{
		/* 
		...
		*/
		if(pPort->bBusVoltage >= MGC_VBUS_ABOVE_AVALID)
		{
			...
			/* signal error */
			if(pOtgClient) 
				pOtgClient->pfOtgError(pOtgClient->pPrivateData, pOtgClient, MUSB_STATUS_OTG_SRP_FAILED);
		}
		else
		{
...
static void MGC_SetHnpEnableIrpComplete(void* pCompleteParam, MUSB_ControlIrp* pIrp)
{
	...
	if(MUSB_STATUS_OK == pIrp->dwStatus)
	{
		pPort->bWantSuspend = TRUE;
		pPort->pfProgramBusState(pPort);
		pServices->pfArmTimer(pServices->pPrivateData, 0, MGC_TA_AIDL_BDIS, FALSE, MGC_AidlBdisTimeout);
	}
}
	...
	*/
	...
	switch(toOtg)
	{
		...
		case MUSB_A_WAIT_BCON:
			pPort->bWantSession = TRUE;
			pPort->pfProgramBusState(pPort);
			pServices->pfArmTimer(pServices->pPrivateData, 0, MUSB_TS_SESSREQ * 1000,
		...
		*/
		...
		case MUSB_A_HOST:
			...
			{
				pServices->pfArmTimer(pServices->pPrivateData, 0, 10, FALSE, MGC_HostResetStart);
			}
		break;
	...
	MGC_DIAG2(2, pPort->pController, "OTG state change from ", pPort->bOtgState, " to ", toOtg, 10, 0);
...
void MGC_CompleteOtgTransition(MGC_Port* pPort)
{
	...
	if(pOtgClient)
	{
		pOtgClient->pfOtgState(pOtgClient->pPrivateData, (MUSB_BusHandle)pPort, (uint8_t)pPort->bOtgState);
	}
}
...
uint8_t MGC_DrcUsbIsr(MGC_Port* pPort, uint8_t bIntrUsbVal)
{
	...
	if(bIntrUsbVal)
	{
		...
		if(bIntrUsbVal & MGC_M_INTR_CONNECT)
		{
			...
			pPort->bIsHost = TRUE;
			bResult++;	
			// 20131209 ZhuoYongHong
			// 连接操作修改为任务中执行，而不是在中断中
			//Nicholas Xu		
			// #if 1
			#if 0	
		...
		if(bIntrUsbVal & MGC_M_INTR_DISCONNECT)
		{
			...
			//#if 1
				// 20131209 ZhuoYongHong
				// MGC_HostDestroy调用任务等待相关的系统方法
				// IAR系统不允许在中断中调用任务等待相关的系统方法
			  #if 0		
		...
		if(bIntrUsbVal & MGC_M_INTR_SOF)
		{
			...
			{
				pEnd = (MGC_EndpointResource*)MUSB_ArrayFetch(&(pPort->LocalEnds), dwEndIndex);
				if(pEnd && (MUSB_ENDPOINT_XFER_ISOC == pEnd->bTrafficType) && pEnd->dwWaitFrameCount)
				{
	...
	while(bOk)
	{
		switch(item.bCause)
		{
				...
				switch (pPort->bOtgState)
				{
					...
					case MUSB_A_SUSPEND:
						...
						pController->pSystemServices->pfArmTimer(
							pController->pSystemServices->pPrivateData, 0, 20, FALSE, MGC_DrcCompleteResume);
					break;
				}
			break;
			...
			case MGC_BSR_CAUSE_DISCONNECT:
				...
				MUSB_PRINTK("Disconnect Interrupt \n");
				/* unequivocally clear this state */
				pPort->bIsSuspend = FALSE;
				
				/*
				// 20131209 ZhuoYongHong
				// Host， 若枚举阶段中，拔去USB，枚举过程中分配的pEnumData->pDevice->pPrivateData没有释放
				if(	pPort->bOtgState == MUSB_A_HOST
					&&	pPort->EnumerationData.bState == 1)
				{
					printf ("host stay in MGC_EnumStateSetAddress\n");
					MGC_EnumerationData* pEnumData = &(pPort->EnumerationData);
					if(pEnumData->pDevice && pEnumData->pDevice->pPrivateData)
					{
						printf ("Host check enumData, pPrivateData=%x\n", pEnumData->pDevice->pPrivateData);
				...
				{
#if 0
					/* NOTE: this transition missing in v1.1 stack */
					case MUSB_A_PERIPHERAL:
					case MUSB_B_PERIPHERAL:
						/*
						MGC_DrcFlushAll(pPort);
			...
			case MGC_BSR_CAUSE_RX_IRP_COMPLETE:
				pLocalEnd = (MGC_EndpointResource*)MUSB_ArrayFetch(&(pPort->LocalEnds), item.bLocalEnd);
#ifdef MUSB_OVERHEAD
			    ...
			    MUSB_DeviceDriver* pDriver)
{
	...
	/* handle test device */
	if((6666 == pDevice->DeviceDescriptor.idVendor) && (0xbadd == pDevice->DeviceDescriptor.idProduct))
	{
		/* inform client */
		if(pOtgClient) 
...
#if 0
/** Wait for valid connector ID callback */
static void MGC_ConnectorIdWait(void* pParam, uint16_t wTimer)
{
	MGC_Controller* pController = (MGC_Controller*)pParam;
	MGC_Port* pPort = pController->pPort;
...
static void MGC_OtgStateGetId(MGC_Port* pPort, uint8_t bIsReset)
{
	...
	if(bIsReset)
	{
		/* if reset, we already have a session; just see if we should be peripheral */
		pPort->pfReadBusState(pPort);
		if(pPort->bConnectorId)
		{
			MGC_DrcChangeOtgState(MUSB_B_PERIPHERAL, pPort);
	...
	{
		/* not from reset; request session and wait for connector ID to be valid */
		pPort->bWantSession = TRUE;
		pPort->pfProgramBusState(pPort);

		/* 50ms wait per UTMI+ */
		pController->pSystemServices->pfArmTimer(
		pController->pSystemServices->pPrivateData, 0, 50, FALSE,
		MGC_ConnectorIdWait);
	}
#endif
	...
	* However, waiting here causes failures in the OPT, so instead we must
	* have some strange transitions as we catch up with the core.
	*/
...
void MUSB_RelinquishHost(MUSB_BusHandle hBus)
{
	...
	else
	{
		...
		pServices->pfArmTimer(pServices->pPrivateData, 0, 10, FALSE, MGC_RemoveSuspend);
	}
}
