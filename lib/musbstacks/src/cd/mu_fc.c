	...
	while(bOk)
	{
		switch(item.bCause)
		{
			case MGC_BSR_CAUSE_RESUME:
				...
				pPort->pFunctionClient->pfUsbState(pPort->pFunctionClient->pPrivateData, (MUSB_BusHandle)pPort, pPort->bUsbState);
			break;
			...
			case MGC_BSR_CAUSE_SUSPEND:
				...
				pPort->pFunctionClient->pfUsbState(pPort->pFunctionClient->pPrivateData, (MUSB_BusHandle)pPort, MUSB_SUSPENDED);
			break;
