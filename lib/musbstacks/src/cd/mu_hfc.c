...
uint8_t MGC_HsfcInit(MGC_Port* pPort)
{
	...
	{
		if(MUSB_ArrayInit(&(pPort->LocalEnds), sizeof(MGC_EndpointResource), MUSB_C_NUM_EPS, pPort->pPrivateData))
		{
			/* add endpoint 0 */
	...
	if(pEnd && bBind)
	{
		...
		if(bIsTx)
		{
			...
			if(bEnd)
			{
				...
				if((MUSB_ENDPOINT_XFER_BULK == pEnd->bTrafficType) && pPort->bHasBulkSplit)
				{
					...
					MGC_WriteCsr16(pBase, MGC_O_HSFC_TXMAXP, bEnd, 
					pEnd->wPacketSize | 
		...
		else
		{
			...
			if(bEnd)
			{
				...
				MGC_WriteCsr16(pBase, MGC_O_HSFC_RXCSR, bEnd, MGC_M_RXCSR_FLUSHFIFO | MGC_M_RXCSR_CLRDATATOG);
				MGC_WriteCsr16(pBase, MGC_O_HSFC_RXCSR, bEnd, MGC_M_RXCSR_FLUSHFIFO | MGC_M_RXCSR_CLRDATATOG);
	...
	do
	{
		...
		*/
		...
		if(pEnd && pEnd->pRxIrp)
		{
			...
			while(pEnd->pRxIrp && ((pEnd->bBsrResidual != pEnd->bIsrResidual) || (wVal & MGC_M_RXCSR_RXPKTRDY)))
			{
...
uint8_t MGC_HsfcLoadFifo(MGC_Port* pPort, uint8_t bEnd, uint32_t dwCount, const uint8_t* pSource)
{
...
uint8_t MGC_HsfcUnloadFifo(MGC_Port* pPort, uint8_t bEnd, uint32_t dwCount, uint8_t* pDest)
{
	uint32_t dwIndex, dwIndex32;
...
*/
int MGC_HsfcDumpState(MGC_Controller* pController)
{
	/* TODO */
	return 0;
}
...
*/
int MGC_HsfcDumpEndpoint(MGC_Controller* pController, MUSB_EndpointResource* pEnd)
{
	/* TODO */
	return 0;
}
