	...
	switch(bTrafficType)
	{
		...
		case MUSB_ENDPOINT_XFER_INT:
			...
			if(pIrp->bIsrCallback)
			{
				...
				{
					if(((MUSB_ENDPOINT_XFER_INT == bTrafficType) || !pEnd->pRxIrp) && !pEnd->bIsRxHalted)
					{
			 ...
			 uint8_t bIsTx)
{
	...
	if(pNextIrp)
	{
		...
		if(pPipe && (bTypeOk || bSwitchBulk))
		{
			...
			{
				pPort->pfProgramStartReceive(pPort, pEnd, pBuffer, dwReqLength, pNextIrp, bAllowDma);
			}
		}
		else
		{
...
uint32_t MUSB_SetPipeHalt(MUSB_Pipe hPipe, uint8_t bHalt)
{
	...
	return pPort->pfProgramHaltEndpoint(pPort, pPipe->pLocalEnd, bIsIn ? MUSB_DIR_IN : 0, bHalt);
}
	...
	uint32_t dwWaitCount, MUSB_IsochIrp* pIsochIrp)
{
	...
	MGC_EndpointResource* pEnd = pPipe->pLocalEnd;
	uint8_t bIsTx = (pPipe->bmFlags & MGC_PIPEFLAGS_TRANSMIT) ? TRUE : FALSE;
	uint8_t bTrafficType = bIsTx ? (pEnd->bTrafficType & MUSB_ENDPOINT_XFERTYPE_MASK) : 
	(pEnd->bRxTrafficType & MUSB_ENDPOINT_XFERTYPE_MASK);
	...
	else
	{
		switch(bTrafficType)
		{
			...
			case MUSB_ENDPOINT_XFER_ISOC:
				...
				if(bIsTx)
				{
					pEnd->bTrafficType &= MUSB_ENDPOINT_XFERTYPE_MASK;
					if(dwWaitCount)
					{
				...
				{
					pEnd->bRxTrafficType &= MUSB_ENDPOINT_XFERTYPE_MASK;
					pPort->pfProgramStartReceive(pPort, pEnd, pBuffer, dwLength, pIsochIrp, pIsochIrp->bAllowDma);
				}
			break;
		}
	}
