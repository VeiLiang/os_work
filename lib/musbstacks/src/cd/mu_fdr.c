				    ...
				    MGC_BsrItem* pItem)
{
	...
	do
	{
...
#endif
					wRxCount = MGC_ReadRxCount(pController);
					bResult = MGC_PipePacketReceived(pPort, status, pEnd, wRxCount, TRUE, pEnd->pRxIrp);
					if(bResult)
					{
#ifdef MUSB_DMA
						if(pEnd->pRxDmaChannel)
		...
		else
		{
			...
			*/
			if((bVal & MGC_M_RXCSR1_P_SENTSTALL) || !(bVal & MGC_M_RXCSR1_RXPKTRDY))
			{
			...
			*/
			...
			{
				/* we are expecting traffic */
				while(pEnd->pRxIrp && ((pEnd->bBsrResidual != pEnd->bIsrResidual) || (bVal & MGC_M_RXCSR1_RXPKTRDY)))
				{
