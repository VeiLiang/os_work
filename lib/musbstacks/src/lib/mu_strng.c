			    ...
			    const char* pPattern, uint8_t bCaseSensitive)
{
	...
	{
		wFoundLength = 0;
		while((dwStringLength >= (int32_t)wPatternLength) && (0 != MGC_CharCompare(*pSearch, *pFind, bCaseSensitive)))
		{
			pSearch++;
			dwStringLength--;
		}
		...
		{
			while((wFoundLength < wPatternLength) && (0 == MGC_CharCompare(pSearch[wFoundLength], pPattern[wFoundLength], bCaseSensitive)))
			{
				wFoundLength++;
			}
		}
