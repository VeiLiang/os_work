...
static void MGC_BotCbwComplete(void* pParam, MUSB_Irp* pIrp);

static void MGC_BotCswComplete(void* pParam, MUSB_Irp* pIrp);

static void MGC_BotDataComplete(void* pParam, MUSB_Irp* pIrp);
				       ...
				       MGC_MsdCmdSet* pCmdSet)
{
	...
	if(!hInPipe)
	{
		MUSB_DIAG_STRING(1, "MSD/BOT: failed to open IN pipe");
		MUSB_PRINTK("MSD/BOT: failed to open IN pipe \n");
		return NULL;
	}
	...
	if(!hOutPipe)
	{
		MUSB_DIAG_STRING(1, "MSD/BOT: failed to open OUT pipe");
		MUSB_PRINTK("MSD/BOT: failed to open OUT pipe \n");
	...
	MUSB_PRINTK("MSD/BOT: failed to allocate instance data \n");
	return NULL;
}
...
void MGC_DestroyBotInstance(MGC_MsdProtocol* pProtocol)
{
	...
	{
		MUSB_CancelControlTransfer(pBot->ControlIrp.pDevice->pPort, &(pBot->ControlIrp));
		...
		for(bIndex = 0; bIndex < pBot->bLunCount; bIndex++)
		{
			...
			if(pVolume && pVolume->pCmdSetInstance)
			{
					    ...
					    const MUSB_HfiDeviceInfo* pInfo)
{
	...
	{
		pVolume->DeviceInfo.awSerialNumber[bIndex] = MUSB_SWAP16P(&(pBot->aData[2+(bIndex << 1)]));
	}
	pVolume->DeviceInfo.awSerialNumber[bIndex] = 0;

	if(pVolume->pCmdSetInstance && !pVolume->bAnnounced && (bLun < (pBot->bLunCount - 1)))
	{
		pBot->pCmdSet->pfDiscoverDevice(pVolume->pCmdSetInstance, &(pBot->Protocol), bLun + 1);
	}
	return TRUE;
}
				    ...
				    uint8_t bIsReady)
{
	...
	if(bIsReady)
	{
		...
		else if(!pVolume->bAnnounced)
		{
			...
			Status = MUSB_HfiAddDevice(&(pVolume->hVolume), &(pVolume->DeviceInfo), &(pVolume->Device));
...
void MGC_BotControlIrpComplete(void* pParam, MUSB_ControlIrp* pIrp)
{
	...
	if(pBot->bInit)
	{
		...
		//Nicholas Xu
		//防止异常情况
		if((pBot->pDevice) && (pBot->pDevice->pPort))
	...
	else if(pBot->bGetSerial)
	{
		...
		pBot->aVolume = (MGC_MsdBotVolume*)MUSB_MemAlloc(pBot->bLunCount * sizeof(MGC_MsdBotVolume));
		if(pBot->aVolume)
		{
			...
			if(pVolume->pCmdSetInstance)
			{
				...
				pBot->pCmdSet->pfDiscoverDevice(pVolume->pCmdSetInstance, &(pBot->Protocol), 0);
			}
		}
	}
...
static void MGC_BotDataComplete(void* pParam, MUSB_Irp* pIrp)
{
	...
	if((pVolume->bTransferStage == MGC_MSD_BOT_TRANSFER_STAGE_DATAIN) && (13 == pIrp->dwActualLength))
	{
...
static void MGC_BotCswComplete(void* pParam, MUSB_Irp* pIrp)
{
	...
	{
		pVolume->bError = TRUE;
		if((MUSB_STATUS_STALLED == pIrp->dwStatus) && !pBot->bCswRetry)
		{
			pBot->bCswRetry = TRUE;
			MUSB_MemCopy(pBot->aSetup, MGC_aMsdBotClearHaltData, sizeof(MGC_aMsdBotClearHaltData));
			pBot->aSetup[4] = pBot->pInEnd->bEndpointAddress;
					      ...
					      MUSB_HfiMediumInfo* pMediumInfo)
{
	...
	MUSB_MemCopy(pMediumInfo, &(pVolume->MediumInfo), sizeof(MUSB_HfiMediumInfo));

	return MUSB_HFI_SUCCESS;
}

static void MGC_MsdBotDataTransferComplete(void* pPrivateData,
					   ...
					   uint16_t wStatusLength)
{
	...
	pfTransferComplete(pVolume->hVolume, dwDataLength / pVolume->DeviceInfo.dwBlockSize);
}
					   ...
					   uint8_t bAllowDma)
{
	...
	if(bCmdLength)
	{
		...
		bOk = MGC_BotProtocolSendCmd(pBot, pVolume, pVolume->bLun, aCmdBuffer,
							bCmdLength, pBuffer, wBlockCount * pVolume->DeviceInfo.dwBlockSize,
...
static MUSB_HfiStatus MGC_MsdBotFlushDevice(void* pDeviceData)
{
	...
	{
		if(!pBot->pCmdSet->pfFlushDevice(pVolume->pCmdSetInstance, &(pBot->Protocol), pVolume->bLun))
		{
			return MUSB_HFI_ERROR_TRANSFER;
		}
	}
...
static MUSB_HfiStatus MGC_MsdBotAbortFormat(void* pDeviceData)
{
	...
	if(pBot->pCmdSet->pfAbortFormat(pVolume->pCmdSetInstance, &(pBot->Protocol), pVolume->bLun))
	{
		pVolume->bFormatting = FALSE;
		return MUSB_HFI_SUCCESS;
	}
...
static MUSB_HfiStatus MGC_MsdBotUnmountVolume(void *pDeviceData)
{
	...
	{
		if(!pBot->pCmdSet->pfUnmountDevice(pVolume->pCmdSetInstance, &(pBot->Protocol), pVolume->bLun))
		{
			return MUSB_HFI_ERROR_TRANSFER;
		}
	}
...
static MUSB_HfiStatus MGC_MsdBotCheckMedium(void* pDeviceData)
{
	...
	{
		if(!pBot->pCmdSet->pfCheckMedium(pVolume->pCmdSetInstance, &(pBot->Protocol), pVolume->bLun))
		{
			return MUSB_HFI_ERROR_TRANSFER;
		}
	}
