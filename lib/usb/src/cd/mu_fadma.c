...
static uint8_t MGC_FaDmaStopController(void* pPrivateData)
{
	...
	{
		MGC_Write16(pBase, MGC_FADMA_CHANNEL_OFFSET(bChannel, MGC_O_FADMA_CONTROL), 0);
...
static void MGC_FaDmaReleaseChannel(MUSB_DmaChannel* pChannel)
{
	...
	MGC_Write16(pBase, MGC_FADMA_CHANNEL_OFFSET(pImplChannel->bIndex, MGC_O_FADMA_CONTROL), 0);
				       ...
				       const uint8_t* pBuffer, uint32_t dwLength)
{
	...
	/* address/count */
	MGC_Write32(pBase, MGC_FADMA_CHANNEL_OFFSET(bChannel, MGC_O_FADMA_ADDRESS), 
	(uint32_t)pController->pfSystemToBusAddress(pController->pSystemPrivate, pBuffer));
	MGC_Write32(pBase, MGC_FADMA_CHANNEL_OFFSET(bChannel, MGC_O_FADMA_COUNT), dwLength);

	/* control (this should start things) */
	pChannel->dwActualLength = 0L;
	pImplChannel->dwStartAddress = (uint32_t)pBuffer;
	pImplChannel->dwCount = dwLength;
	MGC_Write16(pBase, MGC_FADMA_CHANNEL_OFFSET(bChannel, MGC_O_FADMA_CONTROL), wCsr);
...
static uint8_t MGC_FaDmaControllerIsr(void* pPrivateData)
{
	...
	for(bChannel = 0; bChannel < MUSB_FADMA_CHANNELS; bChannel++)
	{
		...
		{
			pImplChannel = (MGC_FaDmaChannel*)&(pController->aChannel[bChannel]);
			wCsr = MGC_Read16(pBase, MGC_FADMA_CHANNEL_OFFSET(bChannel, MGC_O_FADMA_CONTROL));
			dwCount = MGC_Read32(pBase, MGC_FADMA_CHANNEL_OFFSET(bChannel, MGC_O_FADMA_COUNT));
...
#if defined(MUSB_DMA) && defined(MUSB_FADMA) && (MUSB_FADMA_CHANNELS > 0)
	MGC_FaDmaController* pController = (MGC_FaDmaController*)MUSB_MemAlloc(sizeof(MGC_FaDmaController));
	if(pController)
	{
...
{
#if defined(MUSB_DMA) && defined(MUSB_FADMA) && (MUSB_FADMA_CHANNELS > 0)
	MGC_FaDmaController* pFaController = (MGC_FaDmaController*)pController->pPrivateData;
