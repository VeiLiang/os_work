...
void MGC_HdrcSoftReset()
{
	int reset_reg = 0x40408000;
	int val;
	
	val = MGC_Read32(reset_reg, 0x74);
	
	val &= ~((1<<23)|(1<<25));

	MGC_Write32(reset_reg, 0x74, val);
...
unsigned char lg_Usb_Bubble = 0;	//add by helen
/*
	...
	if(nDevCtl != devctl)
	{
		...
		while(1)
		{
			if(pPort->bWantSession)
			{
				...
				if(!Count)
				{
					printk("MUSB The DevCtl Maybe B-Device nDevCtl is 0x%x\n", nDevCtl);
					break;
				}
			}
			else
...
#define USB_FIFO_LOAD_OPT

#ifdef USB_FIFO_LOAD_OPT
static void burst_load_data_from_fifo (volatile unsigned char *addr, unsigned char *data, unsigned int count)
{
	// 使用ARM的LDM指令优化
...
uint8_t MGC_HdrcUnloadFifo(MGC_Port* pPort, uint8_t bEnd, uint32_t dwCount, uint8_t* pDest)
{
	...
	{
		// 4字节对其
		unsigned int burst_count = dwCount & ~0x0F;
		if(burst_count)
		{
			burst_load_data_from_fifo ((volatile unsigned char *)pBase + bFifoOffset, 
												(unsigned char *)pDest, burst_count);
			dwIndex += burst_count;
		}
	}
#else
...
#ifdef USB_FIFO_LOAD_OPT
	{
		unsigned int count = dwCount & (~3);
		while(dwIndex < count)
		{
			unsigned int data = MGC_Read32(pBase, bFifoOffset);
			pDest[dwIndex++] = (unsigned char)(data >> 0);
			pDest[dwIndex++] = (unsigned char)(data >> 8);
			pDest[dwIndex++] = (unsigned char)(data >> 16);
			pDest[dwIndex++] = (unsigned char)(data >> 24);
