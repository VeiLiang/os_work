	...
	if(pEnd && bBind)
	{
		...
		if(bIsTx)
		{
			...
			if(bEnd)
			{
				...
				MGC_WriteCsr16(pBase, MGC_O_MHDRC_TXCSR, bEnd, MGC_M_TXCSR_FLUSHFIFO | MGC_M_TXCSR_CLRDATATOG);
				MGC_WriteCsr16(pBase, MGC_O_MHDRC_TXCSR, bEnd, MGC_M_TXCSR_FLUSHFIFO | MGC_M_TXCSR_CLRDATATOG);
				/* final CSR setup */
			...
			else
			{
				...
				MGC_WriteCsr16(pBase, MGC_O_MHDRC_CSR0, 0, MGC_M_CSR0_FLUSHFIFO);
				MGC_WriteCsr16(pBase, MGC_O_MHDRC_CSR0, 0, MGC_M_CSR0_FLUSHFIFO);
			}
		}
		else
		{
			...
			if(bEnd)
			{
				...
				MGC_WriteCsr16(pBase, MGC_O_MHDRC_RXCSR, bEnd, MGC_M_RXCSR_FLUSHFIFO | MGC_M_RXCSR_CLRDATATOG);
				MGC_WriteCsr16(pBase, MGC_O_MHDRC_RXCSR, bEnd, MGC_M_RXCSR_FLUSHFIFO | MGC_M_RXCSR_CLRDATATOG);
