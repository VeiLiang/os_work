				    ...
				    )
{
	...
	if(bOK)
	{
		...
		for(bEnd = 1; bEnd < pWrapper->PortImpl.bEndCount; bEnd++)
		{
			...
			MUSB_DIAG2(2, "    Tx FIFO size=", pEnd->wMaxPacketSizeTx, " / Rx FIFO size=", pEnd->wMaxPacketSizeRx, 16, 4);
		}
#endif
					      ...
					      uint32_t dwHighWater)
{
	...
	if(pPort->bIsHost)
	{
		...
		for(bBufCount = bEnd = 1; bEnd < pPort->bEndCount; bEnd++)
		{
			...
			if(adwBase[bBufCount])
			{
				...
				MUSB_DIAG2(2, "Dynamic FIFO Tx buffer for end=", bEnd, " / Double-buffered=", abDouble[bBufCount], 10, 0);
				MUSB_DIAG2(2, "\tBase=", adwBase[bBufCount], " / Size=", awSize[bBufCount], 16, 8);
			...
			if(!pEnd->bIsFifoShared)
			{
				...
				if(adwBase[bBufCount])
				{
					...
					MUSB_DIAG2(2, "Dynamic FIFO Rx buffer for end=", bEnd, " / Double-buffered=", abDouble[bBufCount], 10, 0);
					MUSB_DIAG2(2, "\tBase=", adwBase[bBufCount], " / Size=", awSize[bBufCount], 16, 8);
		...
		for(bBufCandidate = 0; bBufCandidate < bBufCount; bBufCandidate++)
		{
			...
			if(dwNearest < dwTotalSize)
			{
				...
				pEnd = (MGC_EndpointResource*)MUSB_ArrayFetch(&(pPort->LocalEnds), bEnd);
				/* stop traffic */
				MUSB_DIAG2(2, "Compacting FIFOs: stopping traffic on end=", bEnd, " / Tx=", abTx[bNearestBuf], 10, 0);
				...
				/* set new location */
				pPort->pfDynamicFifoLocation(pPort, bEnd, abTx[bNearestBuf], FALSE, &dwBase, &bVal, &bDouble);
				
				if(bStealDoubles)
				{
					bDouble = FALSE;
				}
				MUSB_DIAG2(2, "New base=", dwBaseCandidate, " / Double-buffered=", bDouble, 16, 8);
				
				pPort->pfDynamicFifoLocation(pPort, bEnd, abTx[bNearestBuf], TRUE, &dwBaseCandidate, &bVal, &bDouble);
				...
				/* restart traffic */
				pPort->pfProgramHaltEndpoint(pPort, pEnd, abTx[bNearestBuf] ? 0 : MUSB_DIR_IN, FALSE);
			}
		}
	...
	if(bEnd < bEndCount)
	{
		...
		if(bDouble && ((wSize > dwAvailSize) || ((dwAvailSize - wSize) < ((bEndCount - bEnd) * 8))))
		{
		...
		if((wSize < dwAvailSize) && ((dwAvailSize - wSize) >= ((bEndCount - bEnd) * 8)))
		{
			/* we're good to go */
			MUSB_DIAG2(2, "Programming dynamic FIFO end=", bEnd, " / Tx=", bIsTx, 10, 0);
			MUSB_DIAG2(2, "\tBase=", dwBase, " / Effective Size=", wSize, 16, 8);
			
			pPort->pfDynamicFifoLocation(pPort, bEnd, bIsTx, TRUE, &dwBase, &bVal, &bDouble);
