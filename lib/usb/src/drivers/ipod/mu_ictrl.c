...
uint32_t MUSB_IPOD_Control(uint16_t Command, void *Arg)
{
	...
	if(cmd_count)
		//OSSemPend(IpodDeviceSem, 0, OS_OPT_PEND_BLOCKING, &ts, &err);
		OSSemPend(IpodDeviceSem, 0, &err);
	cmd_count = 1;
	...
	switch(Command)
	{
		...
		{
			pIpodDevice->user_event = (UserEvent)Arg;
			dwStatus = 0;
			break;
		}
	...
	//OSSemPost(IpodDeviceSem, OS_OPT_POST_1, &err);
