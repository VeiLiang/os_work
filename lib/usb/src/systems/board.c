...
static void MGC_AfsUdsIsr(void);
...
void uHALr_SetTimerState(uint16_t iTimerIndex, uint8_t OSTmrState, MUSB_pfTimerExpired pfExpired, void* pParam)
{
	Musb_Timer->opt = OS_OPT_TMR_ONE_SHOT;
	Musb_Timer->ulStatus = OSTmrState;
	Musb_Timer->pfnHandler = pfExpired;
	Musb_Timer->pdata = pParam;
}

void uHALr_SetTimerInterval(uint16_t iTimerIndex, uint32_t Interval)
{
	Musb_Timer->ulIntervalTick = Interval/10;	// Time最低为10ms , Interval 单位为ms
}

uint8_t uHALr_GetTimerStatus(uint16_t iTimerIndex)
{
	uint8_t	iTimerStatus;
	
	iTimerStatus = Musb_Timer->ulStatus;
	return iTimerStatus;
}

void uHALr_EnableTimer(uint16_t iTimerIndex)
{
	StartHardwareTimer(Musb_Timer);
}

void uHALr_DisableTimer(uint16_t iTimerIndex)
{
