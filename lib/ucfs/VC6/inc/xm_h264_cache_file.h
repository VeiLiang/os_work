...
#define H264_FATAL	printf

#ifdef H264_DEBUG
#define H264_PRINT	printf
#else
#ifdef WIN32
#define H264_PRINT	
#else
#define H264_PRINT(x,...)	((void)(0))	
#endif
#endif
...
typedef	CRITICAL_SECTION	XM_RSEMA;
...
#define	H264_CACHE_SIZE					0x400000		// 用于Cache块的缓冲区memory字节大小
...
typedef struct _H264_CACHE_BLOCK {
	...
	volatile int	reload_flag;		// 1 标记Cache块从文件中重新加载。
									   //	重新加载时按H264_CACHE_READ_BACK_SIZE大小从文件回读
	// 已重新加载的区域定义
	...
	int			aligned_bytes[6];	
	char			buffer[H264_CACHE_FILE_BLOCK_SIZE];
} H264_CACHE_BLOCK;
...
// 文件系统块分配、写入保护
void XMSYS_file_system_block_write_access_lock (void);
