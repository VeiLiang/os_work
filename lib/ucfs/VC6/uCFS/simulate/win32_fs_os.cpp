//****************************************************************************
//
//	Copyright (C) 2010~2014 ZhuoYongHong
//
//	Author	ZhuoYongHong
//
//	File name: win32_kernel_simulate.c
//	  Win32 系统内核基本服务接口模拟
//
//	Revision history
//
//		2010.08.18	ZhuoYongHong Initial version
//
//****************************************************************************#include <io.h>
#include <stdio.h>
#include <direct.h>
#include <afx.h>
#include <assert.h>
#include <winbase.h>

#include "FS_Int.h"
#include "FS_OS.h"

extern "C" {

static CRITICAL_SECTION *_paSema;
static char       _NumLocks;
static HANDLE     _Event;

/*********************************************************************
*
*       FS_X_OS_Lock
*
*/
void FS_X_OS_Lock(unsigned LockIndex) {
  CRITICAL_SECTION * pSema;

  pSema = _paSema + LockIndex;
  if (pSema) {
    EnterCriticalSection (pSema);
  }
}

/*********************************************************************
*
*       FS_X_OS_Unlock
*
*/
void FS_X_OS_Unlock(unsigned LockIndex) {
  CRITICAL_SECTION * pSema;

  pSema = _paSema + LockIndex;
  if (pSema) {
    LeaveCriticalSection(pSema);
  }
}

/*********************************************************************
*
*       FS_X_OS_Init
*
*  Description:
*    Initializes the OS resources. Specifically, you will need to
*    create four binary semaphores. This function is called by
*    FS_Init(). You should create all resources required by the
*    OS to support multi threading of the file system.
*
*  Parameters:
*    None.
*
*  Return value:
*    0    - on success
*    -1   - on failure.
*/
void FS_X_OS_Init(unsigned NumLocks) {
  unsigned i;
  //OS_RSEMA * pSema;
  CRITICAL_SECTION *pSema;

  _paSema = (CRITICAL_SECTION *)FS_AllocZeroed(NumLocks* sizeof(CRITICAL_SECTION));
  pSema =_paSema;
  for (i = 0; i < NumLocks; i++) {
    InitializeCriticalSection (pSema);
	 pSema ++;
  }
  //OS_EVENT_Create(&_Event);
  _Event = CreateEvent (NULL, FALSE, FALSE, NULL);
}

/*********************************************************************
*
*       FS_X_OS_DeInit
*
*  Description:
*    Delete all locks that have been created by FS_X_OS_Init().
*    This makes sure that a 
*
*  Parameters:
*    None.
*
*/
void FS_X_OS_DeInit(void) {
}

/*********************************************************************
*
*       FS_X_OS_GetTime
*
*/
U32  FS_X_OS_GetTime(void) {
  return (U32)GetTickCount();
}

/*********************************************************************
*
*       FS_X_OS_Wait
*
*  Function description:
*    Wait for an event to be signaled.
*
*  Parameters:
*    Timeout    - Time to be wait for the event object.
*  
*  Return value:
*    0       - Event object was signaled within the timeout value
*   -1       - An error or a timeout occurred.
*/
int FS_X_OS_Wait(int Timeout) {
  int r;

  r = -1;
 // if (OS_EVENT_WaitTimed(&_Event, Timeout) == 0) {
  if(WaitForSingleObject (_Event, Timeout) == WAIT_OBJECT_0){
    r = 0;
  }
  return r;
}

/*********************************************************************
*
*       FS_X_OS_Signal
*
*  Function description:
*    Signals a event
*
*/
void FS_X_OS_Signal(void) {
  // OS_EVENT_Pulse(&_Event);
	PulseEvent (_Event);
}

}