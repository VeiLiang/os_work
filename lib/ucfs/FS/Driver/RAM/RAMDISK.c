...
static int _Read(U8 Unit, U32 SectorNo, void *pBuffer, U32 NumSectors) {
  ...
  FS_MEMCPY(pBuffer, pData, NumSectors * pRAMDISK_PROPS->BytesPerSector);
#ifdef WIN32
  Sleep(_ReadDelay);
#endif
  return 0;
...
static int _Write(U8 Unit, U32 SectorNo, const void *pBuffer, U32 NumSectors, U8 RepeatSame) {
  ...
  if (RepeatSame) {
    do {
      FS_MEMCPY(pData, pBuffer, pRAMDISK_PROPS->BytesPerSector);
      pData += pRAMDISK_PROPS->BytesPerSector;
    } while (--NumSectors);
  } else {
      FS_MEMCPY(pData, pBuffer, pRAMDISK_PROPS->BytesPerSector * NumSectors);
  }
#ifdef WIN32
  Sleep(_WriteDelay);
#endif
