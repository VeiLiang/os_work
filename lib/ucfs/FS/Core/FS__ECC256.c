...
#endif
FS_OPTIMIZE
static int _CalcParity32(U32 Data) {
...
static U32 _ParityToECC(U32 ParLo, U32 ParHi) {
  U32 ecc;
  U32 Necc;
  ecc  = _CalcParity32(ParLo & 0xAAAAAAAA) << 19;   // p1
  ecc |= _CalcParity32(ParLo & 0xCCCCCCCC) << 21;   // p2
  ecc |= _CalcParity32(ParLo & 0xF0F0F0F0) << 23;   // p4
  ecc |= _CalcParity32(ParLo & 0xFF00FF00) << 1;    // p8
  ecc |= _CalcParity32(ParLo & 0xFFFF0000) << 3;    // p16
  ...
  ecc |= Necc;
  return ecc ^ 0xFCFFFF;      // Note: Bits 16 and 17 are not used, therefor 0
}
