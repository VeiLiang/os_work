...
static FS_DIR * _AllocDirHandle(void) {
  ...
  if ((void*)pHandle == NULL) {
    FS_DEBUG_ERROROUT("No directory handle available");
  }
#endif
  return pHandle;
}
...
FS_DIR *FS__OpenDir(const char * pDirName) {
  ...
  if (pVolume) {
  	ret = FS__AutoMount(pVolume);
    if ((ret >= 0)&&(ret & FS_MOUNT_R))  {
...
*
*  Description:
*    API function. Get directory Pos by pDir.
*
*  Parameters:
*    pDir        - Pointer to a FS_DIR data structure.
*
...
U32 FS_GetDirPos(FS_DIR * pDir) {
  ...
  FS_LOCK();
  if(pDir->Dir.DirEntryIndex==0)
	Pos = 0;
  else
    Pos = pDir->Dir.DirPos.DirEntryIndex;
  FS_UNLOCK();
  return Pos;
}
...
*
*  Description:
*    API function. Set directory Pos by pDir.
*
*  Parameters:
*    pDir        - Pointer to a FS_DIR data structure.
*    Pos        - The DirEntryIndex of directory
*
*    Return value:
*    None.
*
...
void FS_SetDirPos(FS_DIR * pDir, U32 Pos) {
  ...
  pDir->Dir.DirPos.DirEntryIndex = Pos;
  FS_UNLOCK();
}
