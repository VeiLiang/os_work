...
static const SIZE_INFO _aSizeInfo[] = {
  {0x0003FFF, 16,   16},  // Up to     8 MBytes
  {0x001FFFF, 32,   32},  // Up to    64 MBytes
  {0x007FFFF, 32,   64},  // Up to   256 MBytes
  {0x01FFFFF, 32,  128},  // Up to  1024 MBytes
  {0x03FFFFF, 64,  128},  // Up to  2048 MBytes
  {0x3FFFFFF, 64, 8192}   // Up to 32768 MBytes
};


static const CHS_INFO _aCHSInfo[] = {
  {0x0000FFF,   2,   16},  // Up to     2 MBytes
  {0x0007FFF,   2,   32},  // Up to    16 MBytes
  {0x000FFFF,   4,   32},  // Up to    32 MBytes
  {0x003FFFF,   8,   32},  // Up to   128 MBytes
  {0x007FFFF,  16,   32},  // Up to   256 MBytes
  {0x00FBFFF,  16,   63},  // Up to   504 MBytes
  {0x01F7FFF,  32,   63},  // Up to  1008 MBytes
  {0x03EFFFF,  64,   63},  // Up to  2016 MBytes
  {0x07DFFFF, 128,   63},  // Up to  4032 MBytes
  {0x07DFFFF, 255,   63},  // Up to 32768 MBytes
};
...
static int _CreatePartition(FS_VOLUME * pVolume, int PartIndex, PART_INFO * pPartInfo) {
  ...
  FS_MEMSET(pBuffer, 0x00, FS__MaxSectorSize);
...
static void _CalcCHSInfo(PART_INFO * pPartInfo, FS_DEV_INFO * pDevInfo) {
  ...
  pPartInfo->StartSector     = Data;

  Data                       = PartFirstSector / (pCHSInfo->NumHeads * pCHSInfo->SectorsPerTrack);
  pPartInfo->StartCylinder   = Data;

  Data                       = PartLastSector % (pCHSInfo->NumHeads * pCHSInfo->SectorsPerTrack);
  Data                      /= pCHSInfo->SectorsPerTrack;
  pPartInfo->EndHead         = Data;

  Data                       = (PartLastSector % pCHSInfo->SectorsPerTrack) + 1;
  pPartInfo->EndSector       = Data;

  Data                       = PartLastSector / (pCHSInfo->NumHeads * pCHSInfo->SectorsPerTrack);
  pPartInfo->EndCylinder     = Data;
...
*       _CalcSizeInfo
*/
static int _CalcFormatInfo(FS_VOLUME * pVolume, VOLUME_FORMAT_INFO * pVolumeFormatInfo) {
  unsigned          i;
  U32               NumClusters;
  ...
  if (pPartInfo->NumSectors4Part < 0x7FA8) {
    FSType = 0x01;
  } else if (pPartInfo->NumSectors4Part < 0x010000) {
    FSType = 0x04;
  } else if (pPartInfo->NumSectors4Part < 0x400000) {
    FSType = 0x06;
  } else if (pPartInfo->NumSectors4Part < 0xFB0400) {
    FSType = 0x0B;
  } else {
    FSType = 0x0C;
  }
...
int FS__SD_Format(FS_VOLUME  * pVolume) {
  ...
  if (pVolume) {
    ...
    if (Status >= 0) {
      ...
      if (_CalcFormatInfo(pVolume, &VolFormatInfo)) {
        return -1;
      }
