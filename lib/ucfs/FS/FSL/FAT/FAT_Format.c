...
/*    NumSectors, SectorsPerCluster, NumRootDirEntries */
  {        256, 0x0001,            0x0020},    /* <= 128kB */
  {        512, 0x0001,            0x0040},    /* <= 256kB */
  {   0x000800, 0x0001,            0x0080},    /* <= 1  MB */
  {   0x001000, 0x0001,            0x0100},    /* <= 2  MB */
  {   0x004000, 0x0002,            0x0100},    /* <=  16 MB */
  {   0x008000, 0x0002,            0x0100},    /* <=  32 MB */
  {   0x040000, 0x0004,            0x0200},    /* <= 128 MB */
  {   0x080000, 0x0008,            0x0200},    /* <= 256 MB */
  {   0x100000, 0x0010,            0x0200},    /* <= 512 MB */
#if (FS_FAT_SUPPORT_FAT32)
  /* media bigger than 512MB are formatted with FAT32 */
  {  0x1000000, 0x0008,            0 },   /* <=  8GB */
  {   33554432, 0x0010,            0 },   /* <= 16GB */
  {   67108864, 0x0020,            0 },   /* <= 32GB */
  { 0xFFFFFFFF, 0x0040,            0 }
#else
  {   0x1FFEA0, 0x0020,            0x0200},   /* <= 1GB */
  {   0x3FFD40, 0x0040,            0x0200},   /* <= 2GB */
/*  {   0x7FFA80, 0x0080,            0x0200},   <= 4GB */
  {   0xFFF500, 0x0080,            0x0200},   /* <= 8GB */
#endif
};
...
static void _UpdatePartTable(FS_VOLUME * pVolume, U32 NumSectors, U8 FATType, U8 * pBuffer) {
  ...
  if (FS_LB_ReadDevice(&pVolume->Partition.Device, 0, pBuffer, FS_SECTOR_TYPE_DATA)) {
    FS_DEBUG_ERROROUT("Could not read sector 0 to update partition 0");
    return;
  }
  ...
  case FS_FAT_TYPE_FAT32:
    break;
  default:
    FS_DEBUG_ERROROUT("Unknown FAT Type");
    return;
#endif
  }
  if (CurrentPartType != PartType) {
    *(pBuffer + Off) = PartType;
    if (FS_LB_WriteDevice(&pVolume->Partition.Device, 0, pBuffer, FS_SECTOR_TYPE_DATA)) {
      FS_DEBUG_ERROROUT("Could not write sector 0 to update partition 0");
    }
  }
}
                        ...
                        U8  UpdatePartition)
{
  ...
  if ((BytesPerSector > FS__MaxSectorSize) || (BytesPerSector == 0)) {
    FS_DEBUG_ERROROUT1("_FatFormatEx: Invalid BytesPerSector value: ", BytesPerSector);
    return -1;
  }
  ...
  if (NumRootDirEntriesProp == 0) {
    if (FATType != FS_FAT_TYPE_FAT32) {
      FS_DEBUG_ERROROUT("_FatFormatEx: NumRootDirEntries illegal.");
      NumRootDirEntriesProp = 16;
    }
  }
#endif
  /*
  ...
  if (FATType != FS_FAT_TYPE_FAT32) {
    ...
    FS_StoreU32LE(&pBuffer[39], 0x01234567);                      /* 32 Bit Volume ID */
    ...
    FS_StoreU32LE(&pBuffer[67], 0x01234567);                      /* 32 Bit Volume ID */
  ...
  i = FS_LB_WriteMultiple(pPart, NumReservedSectors, 2 * FATSize, pBuffer, FS_SECTOR_TYPE_MAN);
  if (i < 0) {
    FS__FreeSectorBuffer(pBuffer);
    return -1;
  }
...
#if (FS_FAT_SUPPORT_FAT32)
  if (FATType == FS_FAT_TYPE_FAT32) {
    FS_StoreU32LE(&pBuffer[4],   0x0FFFFFFF);
    FS_StoreU32LE(&pBuffer[8],   0x0FFFFFFF);
  }
#endif /* FS_FAT_SUPPORT_FAT32 */
  for (i = 0; i < 2; i++) {
  ...
  if (NumRootDirEntries != 0) {
    ...
    j = FS_LB_WriteMultiple(pPart, NumReservedSectors + 2 * FATSize, n, pBuffer, FS_SECTOR_TYPE_DIR);
    if (j < 0) {
      FS__FreeSectorBuffer(pBuffer);
      return -1;
    }
  ...
  if (FATType == FS_FAT_TYPE_FAT32) {
    /* Init FSInfo */
    FS_MEMSET(pBuffer, 0x00, BytesPerSector);
    FS_StoreU32LE(&pBuffer[FSINFO_OFF_SIGNATURE_1],       0x41615252);       /* LeadSig = 0x41615252 */
    FS_StoreU32LE(&pBuffer[FSINFO_OFF_SIGNATURE_2],       0x61417272);       /* StructSig = 0x61417272 */
    FS_StoreU32LE(&pBuffer[FSINFO_OFF_FREE_CLUSTERS],     NumClusters - 1);  /* Invalidate last known free cluster count */
    FS_StoreU32LE(&pBuffer[FSINFO_OFF_NEXT_FREE_CLUSTER], 0x00000003);       /* Give hint for free cluster search */
    FS_StoreU16LE(&pBuffer[510], FAT_SIGNATURE);                             /* Signature = 0xAA55 */
