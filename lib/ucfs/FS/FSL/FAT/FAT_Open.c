...
FS_FAT_DENTRY * FS_FAT_GetDirEntry(FS_VOLUME * pVolume, FS_SB* pSB, FS_DIR_POS * pDirPos) {
  ...
  if (FS__SB_Read(pSB)) {
    FS_DEBUG_ERROROUT("FS_FAT_FindNextDirEntry: Failed to read sector");
    return NULL;
  }
...
*
*  Description:
*    Increments the position in the directoy
*/
  ...
  do {
    ...
    if (!pDirEntry) {
      if ((DirStart == 0) && (pFATInfo->RootEntCnt)) {
        ...
        FS_DEBUG_ERROROUT("FS_FAT_FindEmptyDirEntry: Root directory too small");
        ...
        if (NewCluster) {
          ...
          FS_DEBUG_ERROROUT("FS_FAT_FindEmptyDirEntry: Disk is full");
          return NULL;
        }
      }
      break;
...
*    1             Error:   Name did not comply with 8.3 criterias
*
*  Notes
...
#if FS_SUPPORT_FAT_CHAIN
  if(AccessFlags & FS_FILE_ACCESS_FLAG_R)
  {
	  ClusterCount = FileSize / pVolume->FSInfo.FATInfo.BytesPerCluster;
	  if(FileSize%pVolume->FSInfo.FATInfo.BytesPerCluster)
		  ClusterCount++;
	  CurClusterAbs = FirstCluster;
	  pFile->FATChain[0].FileCluster = 0;
	  pFile->FATChain[0].ClusterAbs = CurClusterAbs;
	  pFile->FATChainCount = 1;
	  for(i = 1; i < ClusterCount; i++)
	  {
          FATEntry = FS_FAT_ReadFATEntry(pVolume, &SB, CurClusterAbs);
          if (SB.HasError)
		  {
              r = 1;             /* Read error */
			  break;
		  ...
		  {
			  CurClusterAbs = FATEntry;
		  }
		  if(CurClusterAbs!=pFile->FATChain[pFile->FATChainCount-1].ClusterAbs + i - pFile->FATChain[pFile->FATChainCount-1].FileCluster)
		  {
			  pFile->FATChain[pFile->FATChainCount].FileCluster = i;
			  pFile->FATChain[pFile->FATChainCount].ClusterAbs = CurClusterAbs;
			  pFile->FATChainCount++;
			  if(pFile->FATChainCount >= FS_MAX_FAT_CHAIN)
			  {
				  r = 1;
				  break;
			  }
