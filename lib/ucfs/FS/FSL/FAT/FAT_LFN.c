...
// 20180511 zhuoyonghong 支持最多10000个短文件名在同一个目录下, 10000*2/60/24=13.9天，可以满足连续13天记录
  #define FS_FAT_LFN_MAX_SHORT_NAME  10000	
#endif
...
static U16 _UTF8_2_Unicode(const char * s) {
  ...
  if ((Char & 0x80) == 0) {                /* Single byte (ASCII)  */
    ...
    FS_DEBUG_ERROROUT("Illegal character during UTF-8 decoding!");
...
static int _CopyString(char * pDest,  const void * p, int MaxLen) {
  ...
  while (MaxLen--) {
    int NumBytes;
    NumBytes = _Unicode_2_UTF8(pDest, *pSrc++);
    if (*pSrc == 0xffff) {
      break;
    }
    pDest += NumBytes;
...
static int _CopyString(char * pDest,  const void * p, int MaxLen) 
{
	...
	{
		halfWord = *pData++;
		halfWord |= (U16)(*pData++ << 8);
 
		//printk("halfWord is %d\r\n", halfWord);
		
		NumBytes = _Unicode_2_UTF8(pDest, halfWord);
		if(halfWord == 0xFFFF)
		{
			break;
		}
...
void Gb2312_2_Unicode(const unsigned char * des, const unsigned char* src, unsigned int size)
{
	int i = 0;
...
//			printk("Gb2312_2_Unicode halfWord = %x\r\n", halfWord);

			unicode = dbcs_2_unicode(halfWord);

			DesData[0] = unicode;
			DesData[1] = unicode>>8;
			
//			printk("Gb2312_2_Unicode %x %x\r\n",DesData[0], DesData[1]);
			i += 2;
			DesData += 2;
		}
		else
...
void Gb2312_2_UTF8(const unsigned char * des, const unsigned char* src, unsigned int size)
{
	unsigned short *UnicodeData;

	UnicodeData = (unsigned short *)kernel_malloc(sizeof(unsigned short)*FS_MAX_PATH);
	if(UnicodeData)
	{
		memset(UnicodeData, 0, sizeof(unsigned short)*FS_MAX_PATH);
		Gb2312_2_Unicode((const unsigned char *)UnicodeData, src, size);
		_CopyString((char *)des, UnicodeData, size);
		kernel_free(UnicodeData);
	}
}
...
static int _CopyLongName(char * pDest, const U8 * pSrc) {
  ...
  //DesData = Dmpmalloc(strlen(pDest)+1);
  //UTF8ToDbcs(pDest, DesData, strlen(pDest));
  //memcpy(pDest, DesData, strlen(pDest));
...
*/
static FS_FAT_DENTRY * _FindDirEntry83(FS_VOLUME * pVolume, FS_SB * pSB, FS_83NAME *pEntryName, U32 DirStart) {
  ...
  do {
    ...
    if (FS_MEMCMP((char*)pDirEntry->data, pEntryName, 11) == 0) { /* Name does match */
      U8 Attrib;
      //
...
*       _LFN_FindDirEntry
*/
static FS_FAT_DENTRY * _LFN_FindDirEntry(FS_VOLUME * pVolume, FS_SB * pSB, const char * sLongName, int Len, U32 DirStart, U8 AttrRequired, int * pLongDirEntryIndex) {
  ...
  do {
    ...
    //
    //  When we found a deleted item, we do not need to analyze it.
    //
    ...
    //
    //  Special case, we found the correct entry, but instead of a short directory entry
    //  a long long entry follows. Therefore we need to start over again.
    //
    if ((CurrentIndex == 0) && pDirEntry->data[DIR_ENTRY_OFF_ATTRIBUTES] == (FS_FAT_ATTR_LONGNAME)) {
    ...
    if (CurrentIndex) {
      ...
      U8 CheckSumShort;
      CheckSumShort = _CalcCheckSum((FS_83NAME*)&pDirEntry->data[0]);
...
*       _LFN_DelLongEntry
*/
static void _LFN_DelLongEntry(FS_VOLUME * pVolume, FS_SB * pSB, U32 DirStart, int LongDirEntryIndex) {
  FS_DIR_POS            DirPos;
  FS_FAT_DENTRY       * pDirEntry;
  int                   NumShortEntries;
...
static FS_FAT_DENTRY * _LFN_CreateDirEntry(FS_VOLUME * pVolume, FS_SB * pSB, const char * pFileName, U32 DirStart, U32 ClusterId, U8 Attribute, U32 Size, U16 Time, U16 Date) {
   ...
   */
  for (i = 1; ; i++) {
    _GenerateShortEntry(&ShortEntry, pFileName, i);
    if (_FindDirEntry83(pVolume, pSB, &ShortEntry, DirStart) == NULL) {
      break;      /* This Short name can be used ! */
    }
    if (++i == FS_FAT_LFN_MAX_SHORT_NAME) {
  ...
  do {
    ...
    if (pDirEntry == NULL) {
      ...
      if ((DirStart == 0) && (pFATInfo->RootEntCnt)) {
        ...
        FS_DEBUG_ERROROUT("FS_FAT_LFN_CreateDirEntry: Root directory too small");
        ...
        if (NewCluster) {
          ...
          FS_DEBUG_ERROROUT("FS_FAT_LFN_CreateDirEntry: Disk is full");
          return NULL;
        }
      }
    }
