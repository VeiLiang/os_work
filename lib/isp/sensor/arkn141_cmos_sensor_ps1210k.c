...
#define	PS1210_LINE_DGAIN		// 使用Integration time (column), 不使用sensor内部的数字增益.
										//		数字增益的精度会导致噪声增加. 使用ERIS代替数字增益

extern int isp_sensor_config_PP1210K (void);

// 20160612 定义Integration time (column)为数字增益, 从1.00( (256 + 0)/256 ) ~  1.99609375( (256 + 255) / 256 )

#define	EXPOSURE_H_LINES_ADDR		(0xc0)	// integration time h	
#define	EXPOSURE_L_LINES_ADDR		(0xc1)	// integration time l	
#define	AGAIN_ADDR						(0xc3)	// globalgain
#define	DGAIN_ADDR						(0xc4)	// digitalgain

#define	STD_30_LINES	1140			// HDTV 2200 X 1125, 74.25MHz, 30fps
#define	STD_30_WIDTH	2200
#define	STD_25_LINES	1350			// 2200 X 1350 X 25 = 74.25MHz
...
static  cmos_inttime_ptr_t cmos_inttime_initialize(void)
{
	...
	cmos_inttime.exposure_ashort = 10;
...
static const u32_t analog_gain_table[112] =
{
...
static cmos_gain_ptr_t cmos_gain_initialize(void)
{
	...
	cmos_gain.max_again_target = 129024;

	// disable again	
	// cmos_gain.again_shift = 0;
	// cmos_gain.max_again_target = 1;	
	
#ifdef PS1210_LINE_DGAIN		// 使用Integration time (column)
	cmos_gain.dgain_shift = 8;
	cmos_gain.max_dgain_target = 511; // 256 + 255
#else
	//cmos_gain.dgain_shift = 6;
	//cmos_gain.max_dgain_target = 0xFF;
	
	// 禁止数字增益
	cmos_gain.dgain_shift = 6;
	cmos_gain.max_dgain_target = 1 << 6;
#endif
...
static void cmos_inttime_gain_update (cmos_inttime_ptr_t p_inttime, cmos_gain_ptr_t gain)
{
	...
	if(p_inttime)
	{
		...
		//printf ("cmos_inttime_update exp_time=%d\n", exp_time);
	
		// Group B 
		arkn141_isp_cmos_sensor_write_register (0x03, 0x01);
		arkn141_isp_cmos_sensor_write_register (EXPOSURE_H_LINES_ADDR, (u8_t)((exp_time & 0xFF00) >> 8));
		arkn141_isp_cmos_sensor_write_register (EXPOSURE_L_LINES_ADDR, (u8_t)(exp_time & 0xFF));
		arkn141_isp_cmos_sensor_write_register (0xC2, (u8_t)(0));
	}
	if(gain)
	{
		// Group B 
		arkn141_isp_cmos_sensor_write_register (0x03, 0x01);
		//arkn141_isp_cmos_sensor_write_register (AGAIN_ADDR, (u8_t)0x01);
		//arkn141_isp_cmos_sensor_write_register (DGAIN_ADDR, (u8_t)0x40);
		arkn141_isp_cmos_sensor_write_register (AGAIN_ADDR, (u8_t)gain->aindex);
		
	#ifdef PS1210_LINE_DGAIN		// 使用Integration time (column)
		arkn141_isp_cmos_sensor_write_register (DGAIN_ADDR, (u8_t)0x40);		// 数字增益固定为1
		arkn141_isp_cmos_sensor_write_register (0xC2, (u8_t)(gain->dindex));	// 设置fractional parts of one line time
	#else	
		arkn141_isp_cmos_sensor_write_register (DGAIN_ADDR, (u8_t)gain->dindex);
	#endif		
	}
}
...
static int cmos_sensor_set_readout_direction (u8_t horz_reverse_direction, u8_t vert_reverse_direction)
{
	...
	if(vert_reverse_direction)
		reg |= (1 << 1);
	arkn141_isp_cmos_sensor_write_register (0x03, 0x00);
	return arkn141_isp_cmos_sensor_write_register (0x05, reg);			// 关闭/开启水平翻转/垂直翻转
}

static const char *ps1210k_cmos_sensor_get_sensor_name (void)
{
	return "PS1210K";	
}

int ps1210k_isp_sensor_init(isp_sen_ptr_t p_sen)
{
	return isp_sensor_config_PP1210K();
}

static void ps1210k_cmos_isp_awb_init (isp_awb_ptr_t p_awb)		// 白平衡初始参数
{
  p_awb->enable = 1;    
  p_awb->mode = 1; //0：算法0，统一灰白平均;  1：算法1，统一色温平均;  2：算法2，区域色温加权
  ...
  p_awb->b2g_min = 256/4;
  p_awb->b2g_max = 256*4;
  p_awb->r2g_light[0] = 161;//154; N 
  p_awb->b2g_light[0] = 210;//195;//192; N
  p_awb->r2g_light[1] = 181;//189; DAY 
  p_awb->b2g_light[1] = 217;//205; DAY
  //p_awb->r2g_light[2] = 186;//197; C 
  //p_awb->b2g_light[2] = 186;//136; C  
  p_awb->r2g_light[2] = 197;//197; C 
  p_awb->b2g_light[2] = 145;//136; C
  p_awb->r2g_light[3] = 276;//305; U 
  p_awb->b2g_light[3] = 133;//131; U
  p_awb->r2g_light[4] = 292;// A   
  p_awb->b2g_light[4] = 138;// A
  p_awb->r2g_light[5] = 0;   
  p_awb->b2g_light[5] = 0;
  p_awb->r2g_light[6] = 0;   
  p_awb->b2g_light[6] = 0;
...
unsigned short gamma_LUT1[] = {       
	 0 ,        776,        1568,       2376,       3200,       4040,       4896,       5768,   
    6656,      7560,       8480,       9416,      10368,      11336,      12320,      13320,   
   14336,      15368,      16416,      17480,      18560,      19656,      20768,      21896,   
   23040,      24200,      25376,      26568,      27776,      29000,      30240,      31496,   
   32768,      33977,      35172,      36353,      37520,      38673,      39812,      40937,   
   42048,      43145,      44228,      45297,      46352,      47393,      48420,      49433,   
   50432,      51417,      52388,      53345,      54288,      55217,      56132,      57033,   
   57920,      58793,      59652,      60497,      61328,      62145,      62948,      63737,   
   64512
};

static int scure[65];
static void ps1210k_cmos_isp_colors_init (isp_colors_ptr_t p_colors)	// 色彩初始参数
{
  int i, j;
  int gamma_wdr[65];
...
*/
  ...
  {
      contrast_Ls = 768; contrast_Hs = 768; 
      
	  k_L = 1024-contrast_Ls; k_H = 1024-contrast_Hs; pmax = 64;
	  t0 = 0; s0=0; t1=32; s1=128; t2=pmax; s2=255; k0 = k_L; k2 = k_H;
	  
	  b = -(s0*256 - s1*256 - k0*t0 + k0*t1)/(t0*t0 - 2*t0*t1 + t1*t1);
	  c = k0 - 2*b*t0; 
	  d = s0*256 - b*t0*t0 - c*t0;
	  bb = (s1*256 - s2*256 - k2*t1 + k2*t2)/(t1*t1 - 2*t1*t2 + t2*t2);
	  cc = k2 - 2*bb*t2; 
	  dd = s1*256 - bb*t1*t1 - cc*t1; 
	  
	  for (i = 0; i <= pmax; i++)
	  {
		  gamma_wdr[i] = ((i >=0) && (i <= t1)) ? (b*i*i + c*i + d) : (bb*i*i + cc*i + dd);
	  }
	  
	  for (j = 0; j <= pmax; j++)
	  {
		  p_colors->gamma.gamma_lut[j] = gamma_wdr[j];
		  scure[j] = gamma_wdr[j];
	  }
...
static void ps1210k_cmos_isp_denoise_init (isp_denoise_ptr_t p_denoise)	// 降噪初始设置
{
  ...
  p_denoise->sensitiv0 = 5; 
  p_denoise->sensitiv1 = 5; 
  p_denoise->sel_3d_table = 1; //2;//1;  
  p_denoise->sel_3d_matrix = 1;

   
  p_denoise->y_thres2 = 14;	//6; 
  ...
  p_denoise->y_thres1 = 14;   
  p_denoise->u_thres1 = 16;
  p_denoise->v_thres1 = 16; 
...
static void ps1210k_cmos_isp_eris_init(isp_eris_ptr_t p_eris)			// 宽动态初始设置
{
  ...
  p_eris->spacev = 0;
  p_eris->target = 128;
  p_eris->black = 4;	//2 * 4;		// 0 ~ 4095
  p_eris->white = 1280;	//4095 - 255;	// 0 ~ 4095
  p_eris->gain_max = 256;
  p_eris->gain_min = 256;
  p_eris->cont_max = 12;
  p_eris->cont_min = 4;
...
#endif
static void ps1210k_cmos_isp_fesp_init(isp_fesp_ptr_t p_fesp)	// 镜头校正, fix-pattern-correction, 坏点去除初始设置
{
  int i, j, k;
...
Y_lenslut[  64] = 28100;
  ...
  p_fesp->Fixpatt.rBlacklevel = 3;
  p_fesp->Fixpatt.grBlacklevel = 3;
  p_fesp->Fixpatt.gbBlacklevel = 3;
  p_fesp->Fixpatt.bBlacklevel = 3;
  ...
  p_fesp->Crosstalk.thresh = 12;
  p_fesp->Crosstalk.snsCgf = 3;		//		值越大, 滤除奇异点的能力越大.
  p_fesp->Crosstalk.thres0cgf = 12;
  p_fesp->Crosstalk.thres1cgf = 12;
...
static void ps1210k_cmos_isp_enhance_init (isp_enhance_ptr_t p_enhance)	// 图像增强初始设置
{
  p_enhance->sharp.enable = 1;
  p_enhance->sharp.mode = 0;
  p_enhance->sharp.coring = 0;//3;// 增强 低频 暗的细节 取值范围： 0-7 
  p_enhance->sharp.strength = 32;//64;//32;//128; 
  p_enhance->sharp.gainmax = 256;
  
  p_enhance->bcst.enable = 0;
  p_enhance->bcst.bright = -8; // -256~255
  //                  设置为1324 导致白色变黑色
  p_enhance->bcst.contrast = 1024;//1024; // 0~1.xxx
  p_enhance->bcst.satuation = 1024; //0~1.xxx
  p_enhance->bcst.hue = 0;	//(8) ; // -128~127
...
static void ps1210k_cmos_isp_ae_init (isp_ae_ptr_t p_ae)		// 自动曝光初始设置
{

}

static void ps1210k_cmos_isp_sys_init (isp_sys_ptr_t p_sys, isp_param_ptr_t p_isp)		// 系统初始设置 (sensor pixel位数, bayer mode)
{
  p_sys->ispenbale = 1;  
  ...
  // 自动测试模式下需要禁止自动丢帧.
  p_sys->vmskenable = 0;// 关闭自动丢帧
//  p_sys->vmskenable = 1;// 使能自动丢帧  
  p_sys->frameratei = 0; 
  p_sys->framerateo = 0;
...
//  p_sys->vifrasel1 = 0x00000000;
...
#if 0// def ISP_ERIS_HIST_VERSION
  // -----------------------------------------
  //
  //  *********** 输出帧率测试 ***************
  //
  ...
  // IN/OUT (2帧/2帧), 2.14, 出现ISP挂起
  p_sys->frameratei = 1;
  p_sys->vifrasel0 = 0x00000001;
  p_sys->vifrasel1 = 0x00000000;
  ...
  // IN/OUT (60帧/45帧, 1.605), 
  p_sys->frameratei = 59;
  p_sys->vifrasel0 = 0xFFFFFFFF;	// 32
  p_sys->vifrasel1 = 0x00000FFF;	// 12
  
  // IN/OUT (60帧/45帧, 1.605),  1.565
  p_sys->frameratei = 59;
  p_sys->vifrasel0 = 0xEEEEEEEE;	// 24
  p_sys->vifrasel1 = 0x0EEEEEEC;	// 20
  
  
  // IN/OUT (64帧/32帧), 1.06 
  p_sys->frameratei = 63;
  p_sys->vifrasel0 = 0xCCCCCCCC;	// 16
  p_sys->vifrasel1 = 0x4CCCCCCC;	// 15
  
  // IN/OUT (30帧/25帧), 1.78
  p_sys->frameratei = 29;
  p_sys->vifrasel0 = 0x0EFEFEFE;	// 24
  p_sys->vifrasel1 = 0;	// 15
  
  // IN/OUT (60帧/55帧, ), 1.95
  p_sys->frameratei = 59;
  p_sys->vifrasel0 = 0xEFFEFFEF;	// 29
...
/*  if(IMAGE_H_SZ == 1920 )
 ...
 // p_sys->imagehblank =500;	// 调整该值("行列场"的列消隐周期)可适配未完成的帧写入. 
  ...
  p_sys->fendIntid[2] = 2;
  p_sys->fendIntid[3] = 3;   
  p_sys->ffiqIntdelay = 50;	// 50*16=800 4;// 快中断 
  p_sys->fendStaid = 0;
  p_sys->infoStadone = 0;

#if 0
  	// 开启3D降噪功能时需要禁止debugmode及RAW输出，同时需要使能refenable
	p_sys->debugmode  = 1;
	p_sys->testenable = 1; //开启dram测试模式  
	p_sys->rawmenable = 0; 
	p_sys->yuvenable  = 1;//1;//0:关掉数据输出  1:打开
	p_sys->refenable  = 1;  
#else // catch raw
	p_sys->debugmode  = 0;
	...
	// work mode
	p_sys->debugmode  = 0;	// 关闭调试
	p_sys->testenable = 0; 	// 关闭dram测试模式  
	p_sys->rawmenable = 0; 	// 关闭RAW数据输出
	p_sys->yuvenable  = 1;	// 开启YUV数据输出  
	p_sys->refenable  = 1;	// 允许参考帧读写(3D降噪使用)
	

	p_sys->yuvformat = isp_get_video_format();  //0：y_uv420 1:y_uv422 2:yuv420 3:yuv422 
   printf("format %d, [0：y_uv420 1:y_uv422 2:yuv420 3:yuv422] \n", p_sys->yuvformat);
...
// ISP运行设置
static void ps1210k_cmos_isp_awb_run (isp_awb_ptr_t p_awb)
{
    int r2g, b2g;
...
static void ps1210k_cmos_isp_colors_run (isp_colors_ptr_t p_colors, isp_awb_ptr_t p_awb, isp_ae_ptr_t p_ae)
{
	
}

static void ps1210k_cmos_isp_denoise_run (isp_denoise_ptr_t p_denoise, isp_ae_ptr_t p_ae)
{
...
static void ps1210k_cmos_isp_eris_run (isp_eris_ptr_t p_eris ,isp_ae_ptr_t p_ae)
{
	int black, light, update, data, gain0, gain1, gain_compasation;	
	int i, j, x0, y0, x1, y1, x2, y2, a, b, c, d, data0, color, decolor_compasation;
	
	update = p_ae->etimeComp == 0; 
	light = p_ae->etimeUpd>>8;
	black = (light <= 256) ? 239  : 
	        (light <= 512) ? 235  : 
	        (light <= 768) ? 232  : 230;
	    
	if ( black != p_eris->resoli )
	{
              p_eris->resoli = (unsigned short)black;
              data0 = (p_eris->enable) | (p_eris->manual<<1) | 
                      (p_eris->resols<<2) | (p_eris->resoli<<8) | 
                      (p_eris->spacev<<16) | (p_eris->target<<24); 
	...
	gain_compasation = 0;     
        gain0 = (256) | ((256)<<16);       
	gain1 = gain0;//(p_ae->lumCurr<16) ? (999) | ((999)<<16) : gain0;    
	
	if ((p_ae->againComp == 1) && (gain_compasation == 1))
    {
    	
	   if (p_ae->againUpd >= p_ae->againMax)
       {	
	      Gem_write ((GEM_ERIS_BASE+0x08), gain1);
	   }
	   else
	   {
	      Gem_write ((GEM_ERIS_BASE+0x08), gain0);	
	   }
    }
	
	decolor_compasation = 1;
	color = (light <= 32) ? 512 : 
	        (light <= 64) ? 512 :
	        (light <= 128) ? 512 :
	        (light <= 256) ? 512 :
...
static void ps1210k_cmos_isp_fesp_run (isp_fesp_ptr_t p_fesp, isp_ae_ptr_t p_ae)
{
    int i, data0, data1;
...
static void ps1210k_cmos_isp_ae_run (isp_ae_ptr_t p_ae)
{
...
u32_t isp_init_cmos_sensor (cmos_sensor_t *cmos_sensor)
{
	...
	cmos_sensor->cmos_sensor_get_sensor_name = ps1210k_cmos_sensor_get_sensor_name;
	// sensor初始化
	cmos_sensor->cmos_isp_sensor_init = ps1210k_isp_sensor_init;
	
	
	cmos_sensor->cmos_isp_awb_init = ps1210k_cmos_isp_awb_init;
	cmos_sensor->cmos_isp_colors_init = ps1210k_cmos_isp_colors_init;
	cmos_sensor->cmos_isp_denoise_init = ps1210k_cmos_isp_denoise_init;
	cmos_sensor->cmos_isp_eris_init = ps1210k_cmos_isp_eris_init;
	cmos_sensor->cmos_isp_fesp_init = ps1210k_cmos_isp_fesp_init;
	cmos_sensor->cmos_isp_enhance_init = ps1210k_cmos_isp_enhance_init;
	cmos_sensor->cmos_isp_ae_init = ps1210k_cmos_isp_ae_init;
	cmos_sensor->cmos_isp_sys_init = ps1210k_cmos_isp_sys_init;

	cmos_sensor->cmos_isp_awb_run = ps1210k_cmos_isp_awb_run;
	cmos_sensor->cmos_isp_colors_run = ps1210k_cmos_isp_colors_run;
	cmos_sensor->cmos_isp_denoise_run = ps1210k_cmos_isp_denoise_run;
	cmos_sensor->cmos_isp_eris_run = ps1210k_cmos_isp_eris_run;
	cmos_sensor->cmos_isp_fesp_run = ps1210k_cmos_isp_fesp_run;
	cmos_sensor->cmos_isp_ae_run = ps1210k_cmos_isp_ae_run;
