...
static void * rbtr_malloc (unsigned int n);
static void rbtr_free (void *p);

#undef malloc
#undef free
#define	malloc	rbtr_malloc
#define	free		rbtr_free

#define	MAX_RBTR_ITEM_COUNT	(1024 * 4)
#define	RBTR_ID		0x52425452	// "RBTR"

typedef struct tag_rbtr_item_s {
	void *prev;
	void *next;
	unsigned int id;
	union {
...
__no_init static rbtr_item_s   rbtr_item_pool[MAX_RBTR_ITEM_COUNT];
static queue_s rbtr_item_free;		// 空闲的单元

void  rbtr_int(void)
{
	int i;
	queue_initialize (&rbtr_item_free);
	for (i = 0; i < MAX_RBTR_ITEM_COUNT; i ++)
	{
		rbtr_item_pool[i].id = RBTR_ID;
		queue_insert ((queue_s *)&rbtr_item_pool[i], &rbtr_item_free);
	}
}
...
static void * rbtr_malloc (unsigned int n)
{
	rbtr_item_s *item = NULL;
	if(n > sizeof(((rbtr_item_s *)0)->u))
	{
		XM_printf ("os fatal error\n");
		assert (0);		
	}
	XM_lock ();
	if(!queue_empty (&rbtr_item_free))
	{
		item = (rbtr_item_s *)queue_delete_next (&rbtr_item_free);
	}
	XM_unlock();
...
static void rbtr_free (void *p)
{
	...
	if(p)
	{
		item = (rbtr_item_s *)(((char *)p) - ((unsigned int)&(((rbtr_item_s *)0)->u)));
		if(item->id != RBTR_ID)
		{
			XM_printf ("os fatal error\n");
			assert (0);			
		...
		XM_lock ();
		queue_insert ( (queue_s *)item, &rbtr_item_free);
		XM_unlock();
	}
}
